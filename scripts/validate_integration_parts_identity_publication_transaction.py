"""Rollback qualification for publication across both integration_parts identities."""
import argparse
import hashlib
import json
...
import scripts.promote_integration_parts_identity_revisions as publisher


def snapshot():
    items=json.loads((ROOT/'docs/reviews/integration_parts_identity_revision_import.json').read_text())['graphs'];ids=[r['artifact_id'] for r in items];result={}
...
if __name__=='__main__':
    ...
    (ROOT/'docs/reviews/integration_parts_identity_publication_transaction.json').write_text(json.dumps(report,indent=2)+'\n')
