"""Activate the qualified original circular two-body period revision at automated Tier 3."""
import argparse
import json
...
from scripts.review_two_body_revision import review,LIMITATIONS
...
RUNNER='two_body-original-revision-community.v1'
DESCRIPTION='Compute the circular Newtonian two-body orbital period from positive SI separation, both masses and explicit scalar gravitational constant. Reuses one approved provider with shape-preserving high-precision evaluation. Original and repaired source histories are preserved. Automated Tier 3 corrected derivation; no literal source-proof parity claim.'
PLAIN_DESCRIPTION='Calculate an orbital period for two isolated point masses in a circular orbit. Supply their separation, both masses and the gravitational constant in SI units. The caller establishes that this model applies; the distance is between the bodies, not either distance from their common centre of mass.'
REFERENCE='openstax-calculus-v3-3-4'
DOMAIN='Isolated Newtonian two point masses in a circular orbit; positive finite SI separation and both masses with identical nonempty shapes, positive scalar G and positive finite period. Caller establishes physical applicability. No relativistic, extended-body or external perturbation correction.'
...
def promote(source,apply=False):
    if apply:
        gates=json.loads((ROOT/'docs/reviews/two_body_revision_publication_transaction.json').read_text())
        require(gates['passed'] and gates['post_activation_failure_rolled_back']
            and gates['publisher_sha256']==sha(__file__)
            and gates['validator_sha256']==sha(ROOT/'scripts/validate_two_body_publication_transaction.py'),'Publication rollback qualification required')
    ...
    with psycopg.connect(dotenv_values(ROOT/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row,
        ...
        db.execute("SELECT pg_advisory_xact_lock(hashtext('two_body-original-revision-draft.v1'))")
        ...
        bound=uuid5(version,RUNNER+':circular-two-body-regime')
        ...
        if not already:
            updated+=db.execute("UPDATE artifacts SET status='approved',is_publishable=true,description=%s,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=4,top_level_output_arity=1,topo_hash=%s WHERE artifact_id=%s",(DESCRIPTION,topology,target)).rowcount
...
if __name__=='__main__':
    ...
    if args.apply:
        name='two_body_revision_publication_repeat.json' if report['already_approved'] else 'two_body_revision_publication.json'
