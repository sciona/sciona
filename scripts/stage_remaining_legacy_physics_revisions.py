...
from scripts.audit_remaining_legacy_physics_scope import ROOT,audit,require
from scripts.import_residual_execution_drafts import ensure_row
from sciona.services.execution_graph_codec import encode_execution_graph
from sciona.services.catalog_artifact_retrieval import _artifact_document_to_cdg

RUNNER='remaining-legacy-physics-execution-draft.v1'
...
def stage(source,apply=False,fail_after_writes=None):
    if apply:
        gates=json.loads((ROOT/'docs/reviews/remaining_legacy_physics_revision_transaction.json').read_text())
        require(gates['passed'] and gates['rollback_verified'] and gates['injected_failure_rollback_verified']
            and gates['stager_sha256']==sha(__file__) and gates['validator_sha256']==sha(ROOT/'scripts/validate_remaining_legacy_physics_transaction.py'),'Current transaction qualification required')
    scope=audit(source)
    require(scope==json.loads((ROOT/'docs/reviews/remaining_legacy_physics_execution_scope.json').read_text()),'Qualified legacy scopes changed')
    created=0;reports=[]
        ...
        for item in sorted(scope['graphs'],key=lambda r:r['legacy_artifact_id']):
            ...
            for direction,ports in [('input',graph.nodes[0].inputs),('output',graph.nodes[0].outputs)]:
                ...
                binding_confidence=1.,binding_source=RUNNER,alternatives=Jsonb([]),evidence_summary=Jsonb(dict(
                    qualified_parent_evidence=parent_evidence,qualified_parent_version_id=qualified['version_id'],legacy_scope_sha256=sha(ROOT/'docs/reviews/remaining_legacy_physics_execution_scope.json')))))
    ...
    return dict(applied=apply,approved=False,rows_created=created,graphs=reports,stager_sha256=sha(__file__),
        scope_audit_sha256=sha(ROOT/'docs/reviews/remaining_legacy_physics_execution_scope.json'))
...
if __name__=='__main__':
    ...
    if args.apply:
        name='remaining_legacy_physics_revision_repeat.json' if report['rows_created']==0 else 'remaining_legacy_physics_revision_import.json'
