...
from scripts.promote_series_revision import check_publication,DESCRIPTION,PLAIN_DESCRIPTION
from scripts.review_series_revision import review
from scripts.validate_series_revision_execution import validate
...
async def verify(source):
    ...
    return dict(passed=True,approved=True,trust_tier=3,selection=before,execution=execution,catalog_mutations=0,
        validator_sha256=sha(__file__),execution_validator_sha256=sha(ROOT/'scripts/validate_series_revision_execution.py'),
...
if __name__=='__main__':
    ...
    (ROOT/'docs/reviews/series_revision_served_verification.json').write_text(json.dumps(report,indent=2)+'\n')
    print(json.dumps(dict(passed=True,approved=True,trust_tier=3,independent_circuit_cases=256,invalid_input_rejections=10)))
