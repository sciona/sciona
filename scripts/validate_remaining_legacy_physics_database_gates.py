...
from scripts.stage_remaining_legacy_physics_revisions import ROOT,sha,require,RUNNER
from scripts.check_remaining_legacy_physics_revision import check


def cases(item):
    version=item['version_id'];original=item['original_version_id'];artifact=item['artifact_id']
    return [
...
def validate():
    items=json.loads((ROOT/'docs/reviews/remaining_legacy_physics_revision_import.json').read_text())['graphs'];reports=[]
    ...
    return dict(passed=True,approved=False,rollback_verified=True,committed_catalog_mutations=0,graphs=reports,
        ...
        scope='Fourteen faults per legacy revision, plus fresh intact reads. Qualified parent provider/contract/corruption gates remain bound through each family publication review.')


if __name__=='__main__':
    report=validate()
    (ROOT/'docs/reviews/remaining_legacy_physics_database_gates.json').write_text(json.dumps(report,indent=2)+'\n')
    print(json.dumps(dict(passed=True,graphs=5,rejected_faults=sum(len(r['rejected_faults']) for r in report['graphs']),committed_catalog_mutations=0)))
