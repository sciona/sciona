...
from scripts.stage_period_frequency_original_revision import ROOT,ARTIFACT,ORIGINAL,RUNNER,history,stage,sha,require
...
def validate(source):
    ...
    return dict(passed=True,rollback_verified=True,injected_failure_rollback_verified=True,
        ...
        stager_sha256=sha(ROOT/'scripts/stage_period_frequency_original_revision.py'),validator_sha256=sha(__file__))
...
if __name__=='__main__':
    ...
    (ROOT/'docs/reviews/period_frequency_original_revision_transaction.json').write_text(json.dumps(report,indent=2)+'\n')
