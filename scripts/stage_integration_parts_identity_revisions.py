"""Stage both original integration_parts identities with their qualified symbolic realization."""
import argparse
import hashlib
import importlib
import json
...
from scripts.audit_integration_parts_identity_scope import ROOT,audit,require,check_parent
from scripts.import_residual_execution_drafts import ensure_row
from sciona.services.execution_graph_codec import encode_execution_graph
from sciona.services.catalog_artifact_retrieval import _artifact_document_to_cdg

RUNNER='integration_parts-original-identities-draft.v1'
SCOPE='Corrected parametrized integration-by-parts identity for scalar commutative C1 functions on a caller-established common connected real interval; independent constant and residual integral retained; original incomplete source AST remains immutable provenance.'
...
def stage(source,apply=False,fail_after_writes=None):
    if apply:
        gates=json.loads((ROOT/'docs/reviews/integration_parts_identity_revision_transaction.json').read_text())
        require(gates['passed'] and gates['rollback_verified'] and gates['injected_failure_rollback_verified']
            and gates['stager_sha256']==sha(__file__) and gates['validator_sha256']==sha(ROOT/'scripts/validate_integration_parts_identity_transaction.py'),'Current transaction qualification required')
    scope=audit(source)
    require(scope==json.loads((ROOT/'docs/reviews/integration_parts_identity_execution_scope.json').read_text()),'Qualified legacy scopes changed')
    created=0;reports=[]
    with psycopg.connect(dotenv_values(ROOT/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row,
        ...
        for item in sorted(scope['graphs'],key=lambda r:r['legacy_artifact_id']):
            ...
            parent,qualified=check_parent(db,item['qualification'])
            ...
            for direction,ports in [('input',graph.nodes[0].inputs),('output',graph.nodes[0].outputs)]:
                ...
                binding_confidence=1.,binding_source=RUNNER,alternatives=Jsonb([]),evidence_summary=Jsonb(dict(
                    qualified_parent_evidence=parent_evidence,qualified_parent_version_id=qualified['version_id'],legacy_scope_sha256=sha(ROOT/'docs/reviews/integration_parts_identity_execution_scope.json')))))
            for selected in dict.fromkeys([original,item['projected_source_version_id'],qualified['version_id']]):
    ...
    return dict(applied=apply,approved=False,rows_created=created,graphs=reports,stager_sha256=sha(__file__),
        scope_audit_sha256=sha(ROOT/'docs/reviews/integration_parts_identity_execution_scope.json'))
...
if __name__=='__main__':
    ...
    if args.apply:
        name='integration_parts_identity_revision_repeat.json' if report['rows_created']==0 else 'integration_parts_identity_revision_import.json'
