...
from scripts.promote_nasa_first_lifecycle_providers import RUNNER, REFERENCE
...
def verify():
    ...
    with psycopg.connect(dotenv_values(ROOT/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        for atom in proposed['atoms']:
            ...
            expected = dict(details=dict(qualification=qualification, binding=atom, publication_tier=3,
                publisher_sha256=sha(ROOT/'scripts/promote_nasa_first_lifecycle_providers.py')),
    ...
    return dict(passed=True, read_only=True, approved=True, trust_tier=3, served_providers=len(rows),
        ...
        limitations=['Fresh SQL served-view, version, contract and approval verification; not HTTP transport or ranking qualification.',
            'Stored-contract execution and generic cross-domain reuse are bound in the review; original workflow publication and served-CDG retrieval remain pending.'])


if __name__ == '__main__':
    report = verify()
    (ROOT/'docs/reviews/competition_nasa_first_lifecycle_served.json').write_text(json.dumps(report, indent=2)+'\n')
