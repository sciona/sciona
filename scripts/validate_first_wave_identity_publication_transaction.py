"""Rollback qualification for first-wave publication, including post-activation failure."""
import argparse
import hashlib
import json
...
import scripts.promote_first_wave_identity_revisions as publisher


def snapshot():
    items=json.loads((ROOT/'docs/reviews/first_wave_identity_revision_import.json').read_text())['graphs'];ids=[r['artifact_id'] for r in items];result={}
...
def validate(source):
    ...
    def fail_after_write(*args,**kwargs):
        ...
        if writes==3:raise RuntimeError('injected cross-family publication failure')
        return result
    ...
    require(writes==3 and snapshot()==before,'Cross-family rollback differs')
...
if __name__=='__main__':
    ...
    (ROOT/'docs/reviews/first_wave_identity_publication_transaction.json').write_text(json.dumps(report,indent=2)+'\n')
