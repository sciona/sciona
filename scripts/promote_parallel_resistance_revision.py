"""Activate the qualified original reconstructed parallel_resistance revision at automated Tier 3."""
import argparse
import json
...
from scripts.review_parallel_resistance_revision import review,LIMITATIONS
...
RUNNER='parallel_resistance-original-revision-community.v1'
DESCRIPTION='Evaluate equivalent resistance, both branch currents and total current for two ideal positive linear resistors across common nodes. Reuses one approved provider with exact rational intermediate arithmetic and independent float64 output rounding. Three SI input arrays have identical nonempty shapes; signed common voltage includes zero. Automated Tier 3; recovered source equation and zero-voltage extension remain explicit.'
PLAIN_DESCRIPTION='Calculate the equivalent resistance and branch and total currents of two resistors connected in parallel. Supply both positive resistances in ohms and their common voltage in volts. Supports either voltage polarity and zero voltage. Assumes ideal linear resistors; does not model reactive components, shorts or open circuits.'
REFERENCE='openstax-two-parallel-resistors'
DOMAIN='Two ideal positive finite linear resistors across common nodes with consistently oriented currents. Finite signed common voltage, including zero, in SI units and identical nonempty array shapes. Zero-voltage behavior follows the constitutive model rather than division by zero. Exact converted-input arithmetic before independent float64 rounding; no negative, nonlinear, reactive, ideal short or open circuit scope.'
...
def promote(source,apply=False):
    if apply:
        gates=json.loads((ROOT/'docs/reviews/parallel_resistance_revision_publication_transaction.json').read_text())
        require(gates['passed'] and gates['post_activation_failure_rolled_back']
            and gates['publisher_sha256']==sha(__file__)
            and gates['validator_sha256']==sha(ROOT/'scripts/validate_parallel_resistance_publication_transaction.py'),'Publication rollback qualification required')
    ...
    with psycopg.connect(dotenv_values(ROOT/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row,
        ...
        db.execute("SELECT pg_advisory_xact_lock(hashtext('parallel_resistance-original-revision-draft.v1'))")
        ...
        bound=uuid5(version,RUNNER+':ideal-parallel_resistance-regime')
        ...
        if not already:
            updated+=db.execute("UPDATE artifacts SET status='approved',is_publishable=true,description=%s,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=3,top_level_output_arity=4,topo_hash=%s WHERE artifact_id=%s",(DESCRIPTION,topology,target)).rowcount
...
if __name__=='__main__':
    ...
    if args.apply:
        name='parallel_resistance_revision_publication_repeat.json' if report['already_approved'] else 'parallel_resistance_revision_publication.json'
