...
import importlib
import json
from scripts.stage_remaining_legacy_physics_revisions import ROOT,history,revised_graph,sha,require
from sciona.services.execution_graph_codec import encode_execution_graph
from sciona.services.catalog_artifact_retrieval import _artifact_document_to_cdg


def check(db,family,approved=False):
    imported=json.loads((ROOT/'docs/reviews/remaining_legacy_physics_revision_import.json').read_text())
    require(imported['stager_sha256']==sha(ROOT/'scripts/stage_remaining_legacy_physics_revisions.py'),'Legacy stager changed')
    require(imported['scope_audit_sha256']==sha(ROOT/'docs/reviews/remaining_legacy_physics_execution_scope.json'),'Legacy scope evidence changed')
    items=[r for r in imported['graphs'] if r['family']==family]
    scopes=[r for r in json.loads((ROOT/'docs/reviews/remaining_legacy_physics_execution_scope.json').read_text())['graphs'] if r['family']==family]
    ...
    parent,qualified=publisher.check_publication(db,scope['qualification'])
    ...
    deps=[]
    for selected in [item['original_version_id'],scope['projected_source_version_id'],qualified['version_id']]:
