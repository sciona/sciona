"""Version-bound Tier 3 review of both corrected sine_double_angle source identities."""
import json
import psycopg
from dotenv import dotenv_values
from psycopg.rows import dict_row
from scripts.stage_sine_double_angle_identity_revisions import ROOT,sha,require
from scripts.audit_sine_double_angle_identity_scope import audit
from scripts.check_sine_double_angle_identity_revision import check
from scripts.validate_sine_double_angle_identity_database_gates import cases
from scripts.promote_sine_double_angle_execution import LIMITATIONS,REFERENCE

DESCRIPTION='Compute the sine of twice a finite real radian angle using the reviewed product identity and high-precision arithmetic before float64 rounding. Explicit source imaginary-unit interpretation; original classifications remain provenance. Scalar and array shape are preserved.'
PLAIN_DESCRIPTION='Calculate sin(2x) for finite real angles in radians, preserving the input shape. Inputs are converted to float64; degrees and complex angles are outside the supported contract.'



def review(source):
    directory=ROOT/'docs/reviews';scope=audit(source)
    require(scope==json.loads((directory/'sine_double_angle_identity_execution_scope.json').read_text()),'Fresh sine_double_angle scope changed')
    reports={name:json.loads((directory/('sine_double_angle_identity_'+name+'.json')).read_text()) for name in
    ...
    families={'sine_double_angle_legacy','sine_double_angle_projected'}
    for report in [scope,imported,repeat,execution,gates]:
        require(len(report['graphs'])==2 and {r['family'] for r in report['graphs']}==families,'Both sine_double_angle identities required')
    ...
    require(transaction['passed'] and transaction['rollback_verified'] and transaction['injected_failure_rollback_verified']
        and transaction['stager_sha256']==sha(ROOT/'scripts/stage_sine_double_angle_identity_revisions.py')
        and transaction['validator_sha256']==sha(ROOT/'scripts/validate_sine_double_angle_identity_transaction.py'),'Staging rollback changed')
    require(execution['passed'] and not execution['approved'] and execution['catalog_mutations']==0
        and execution['validator_sha256']==sha(ROOT/'scripts/validate_sine_double_angle_identity_execution.py')
        and execution['checker_sha256']==sha(ROOT/'scripts/check_sine_double_angle_identity_revision.py')
        and execution['oracle_sha256']==sha(ROOT/'scripts/validate_sine_double_angle_execution.py')
        and execution['source_validator_sha256']==sha(ROOT/'scripts/validate_sine_double_angle_immutable_source.py'),'Exact execution code required')
    require(gates['passed'] and gates['rollback_verified'] and gates['committed_catalog_mutations']==0
        and gates['validator_sha256']==sha(ROOT/'scripts/validate_sine_double_angle_identity_database_gates.py')
    ...
    with psycopg.connect(dotenv_values(ROOT/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row,
        ...
        for item in imported['graphs']:
            ...
            require(result['graph_digest']==item['graph_sha256'] and result['full_runner_cases']==6 and result['synthetic_states']==29
                and result['maximum_ulp_error']==0 and result['source_proof']==scope['fresh_execution']['source_proof'],'Independent trigonometric/source coverage differs')
            ...
            require(len(refs)==len(bounds)==1 and bounds[0]['evidence_ref_key']==REFERENCE,'Unique reviewed sine_double_angle regime/reference required')
    ...
    return dict(eligible_for_approval_transaction=True,approved=False,proposed_tier=3,review_source='automated',catalog_mutations=0,graphs=reviewed,
        evidence_sha256={'sine_double_angle_identity_'+name+'.json':sha(directory/('sine_double_angle_identity_'+name+'.json')) for name in names},
        reviewer_sha256=sha(__file__),parent_policy_sha256=sha(ROOT/'scripts/promote_sine_double_angle_execution.py'))
...
if __name__=='__main__':
    ...
    (ROOT/'docs/reviews/sine_double_angle_identity_publication_review.json').write_text(json.dumps(report,indent=2)+'\n')
