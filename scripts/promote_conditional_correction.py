"""Atomically approve the qualified correction drafts at automated Tier 3."""
import argparse
import json
...
from scripts.review_conditional_correction_publication import review, LIMITATIONS
from scripts.validate_conditional_correction_database_gates import check_staged

RUNNER = 'conditional-correction-community.v1'


def promote(apply=False):
    if apply:
        gate_path = ROOT / 'docs/reviews/conditional_correction_publication_transaction_gates.json'
        gates = json.loads(gate_path.read_text())
        require(gates['passed'] is True and gates['publisher_sha256'] == sha(Path(__file__))
                and gates['validator_sha256'] == sha(ROOT / 'scripts/validate_conditional_correction_publication_transaction.py')
    ...
    with psycopg.connect(dotenv_values(ROOT / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        db.execute("SELECT pg_advisory_xact_lock(hashtext('conditional-correction-draft.v1'))")
        ids = [identity] + [UUID(b['artifact_id']) for b in proposed['bindings']]
        states = db.execute('SELECT status,is_publishable FROM artifacts WHERE artifact_id=ANY(%s::uuid[]) FOR UPDATE', (ids,)).fetchall()
        require(len(states) == 3, 'Staged artifacts missing')
        ...
        reference = 'conditional-correction-drivendata-source'
        url = 'https://github.com/drivendataorg/nasa-airport-pushback/tree/09f2f5d2940dd6b63b93115a0e9fb9e8a964c700'
        title = 'Pinned NASA third-place conditional correction source'
        ...
        targets = [(UUID(b['artifact_id']), UUID(b['version_id']), b) for b in proposed['bindings']] + [(identity, version, None)]
        for target, selected, binding in targets:
            for table, key in [('artifact_references', 'artifact_id')] + ([('atom_references', 'atom_id')] if binding else []):
                    ...
                    'relevance_note': 'Pinned numerical source operations; retained MIT notice, float64 source comparison and synthetic cross-domain execution.'})
            ...
            details = dict(qualification=qualification, publisher_sha256=sha(Path(__file__)), publication_tier=3,
                           scope='Provisioned in-process reusable numerical correction', binding=binding)
        ...
        if not already:
            db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=2,"
                       'top_level_input_arity=6,top_level_output_arity=1,topo_hash=%s WHERE artifact_id=%s', (topology, identity))
    ...
    return dict(applied=apply, already_approved=already, rows_created=created, trust_tier=3,
                served_atoms_in_transaction=2, served_cdgs_in_transaction=1, version_id=str(version), graph_sha256=proposed['graph_sha256'])
