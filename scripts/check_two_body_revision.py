...
from scripts.stage_two_body_original_revision import ROOT,ARTIFACT,ORIGINAL,REPLAY,EXECUTION,history,sha,require
...
def check(db,approved=False):
    ...
    require(imported['stager_sha256']==sha(ROOT/'scripts/stage_two_body_original_revision.py'),'Stager changed')
    require(imported['scope_audit_sha256']==sha(ROOT/'docs/reviews/physics_two_body_original_scope.json'),'Scope evidence changed')
    ...
    require(len(bindings)==1 and bindings[0]['node_id']=='period' and bindings[0]['status']=='active'
    ...
    approvals=db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='two_body-corrected-community.v1' AND passed",(EXECUTION,)).fetchall()
    require(len(approvals)==1,'Parent semantic approval missing')
    evidence=ROOT/'docs/reviews/two_body_execution.json'
    require(sha(evidence)==approvals[0]['details']['evidence_sha256']['two_body_execution.json'],'Parent evidence changed')
    qualified=json.loads(evidence.read_text())
    require(sha(inspect.getfile(circular_orbital_period))==qualified['provider_sha256'],'Provider source changed')
    require(list(inspect.signature(circular_orbital_period).parameters)==['separation_metres','mass1_kg','mass2_kg','gravitational_constant'],'Provider signature differs')
