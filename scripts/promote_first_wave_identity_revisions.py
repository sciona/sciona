"""Atomically activate the qualified first-wave revision at automated Tier 3."""
import argparse
import importlib
import json
...
from scripts.stage_first_wave_identity_revisions import ROOT,sha,require,RUNNER as DRAFT_RUNNER
from scripts.review_first_wave_identity_publication import review
...
RUNNER='first-wave-original-identity-community.v1'
...
def promote(source,apply=False):
    if apply:
        gates=json.loads((ROOT/'docs/reviews/first_wave_identity_publication_transaction.json').read_text())
        require(gates['passed'] and gates['post_activation_failure_rolled_back'] and gates['publisher_sha256']==sha(__file__)
            and gates['validator_sha256']==sha(ROOT/'scripts/validate_first_wave_identity_publication_transaction.py'),'Qualified publication rollback required')
...
if __name__=='__main__':
    ...
    if args.apply:
        name='first_wave_identity_publication_repeat.json' if report['already_approved'] else 'first_wave_identity_publication.json'
