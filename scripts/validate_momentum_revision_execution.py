"""Run actual stored momentum revision with immutable source and Decimal oracle."""
import argparse
import asyncio
import json
...
from scripts.stage_momentum_original_revision import ROOT,sha,require
from scripts.check_momentum_revision import check
from scripts.audit_momentum_original_scope import audit
from scripts.validate_momentum_immutable_source import validate as validate_source
import scripts.validate_momentum_execution as oracle
...
async def validate(source,approved=False):
    scope=audit(source)
    require(scope==json.loads((ROOT/'docs/reviews/physics_momentum_original_scope.json').read_text()),'Qualified scope changed')
    ...
    with patch.object(oracle,'build_momentum_execution',return_value=graph),patch.object(oracle,'validate_source',side_effect=validate_source):
    ...
    require(execution['graph_digest']==imported['graph_sha256'] and execution['full_runner_cases']==5
        and execution['synthetic_vectors']==84 and execution['maximum_ulp_error']==0,'Stored execution coverage differs')
    ...
    return dict(passed=True,approved=approved,catalog_mutations=0,version_id=imported['version_id'],graph_sha256=imported['graph_sha256'],
        original_history_preserved=True,stored_execution=execution,validator_sha256=sha(__file__),checker_sha256=sha(ROOT/'scripts/check_momentum_revision.py'),
        source_validator_sha256=sha(ROOT/'scripts/validate_momentum_immutable_source.py'),scope_audit_sha256=imported['scope_audit_sha256'],
        limitations=['Reconstructed Euclidean momentum proof with explicit physical premises; no literal source-rule parity, energy or full scattering event validation.',
...
if __name__=='__main__':
    ...
    name='momentum_revision_served_execution.json' if args.approved else 'momentum_revision_catalog_execution.json'
    (ROOT/'docs/reviews'/name).write_text(json.dumps(report,indent=2)+'\n')
    print(json.dumps(dict(passed=True,approved=args.approved,stored_vectors=84,maximum_ulp_error=0)))
