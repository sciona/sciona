...
def check_staged(db, proposed, *, approved=False):
    ...
    require(_artifact_document_to_cdg(document, version_id=proposed['version_id'], content_hash=proposed['graph_sha256'],
            require_execution_envelope=True) == build_residual_classifier_graph(), 'Stored graph differs')
...
def validate():
    ...
    with psycopg.connect(dotenv_values(ROOT / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        check_staged(db, proposed)
    return dict(format='residual-classifier-database-gates.v1', passed=True, rejected_faults=rejected,
...
if __name__ == '__main__':
    report = validate()
    (ROOT / 'docs/reviews/residual_classifier_database_gates.json').write_text(json.dumps(report, indent=2) + '\n')
