"""Rollback qualification for publication across all five legacy identities."""
import argparse
import hashlib
import json
...
import scripts.promote_remaining_legacy_physics_revisions as publisher


def snapshot():
    items=json.loads((ROOT/'docs/reviews/remaining_legacy_physics_revision_import.json').read_text())['graphs'];ids=[r['artifact_id'] for r in items];result={}
...
if __name__=='__main__':
    ...
    (ROOT/'docs/reviews/remaining_legacy_physics_publication_transaction.json').write_text(json.dumps(report,indent=2)+'\n')
