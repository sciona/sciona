...
from scripts.stage_parallel_resistance_original_revision import ROOT,ARTIFACT,sha,require
from scripts.promote_parallel_resistance_revision import check_publication,DESCRIPTION,PLAIN_DESCRIPTION
from scripts.review_parallel_resistance_revision import review
from scripts.validate_parallel_resistance_revision_execution import validate
...
async def verify(source):
    ...
    return dict(passed=True,approved=True,trust_tier=3,selection=before,execution=execution,catalog_mutations=0,
        validator_sha256=sha(__file__),execution_validator_sha256=sha(ROOT/'scripts/validate_parallel_resistance_revision_execution.py'),
...
if __name__=='__main__':
    ...
    (ROOT/'docs/reviews/parallel_resistance_revision_served_verification.json').write_text(json.dumps(report,indent=2)+'\n')
