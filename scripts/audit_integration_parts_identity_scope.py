"""Qualify both integration_parts originals for explicitly bounded corrected reuse."""
import asyncio
...
from scripts.check_integration_parts_identity_parent import check_parent
...
from sciona.physics_ingest.integration_parts_proof import SOURCE_VERSION
...
async def numerical(source):
    ...
    original=json.loads((ROOT/'docs/reviews/integration_parts_execution.json').read_text())
    normalized=json.loads(json.dumps(result));normalized['source_proof']['validator_sha256']=original['source_proof']['validator_sha256']
    require(normalized==original,'Fresh immutable integration_parts execution differs')
    return result
...
def audit(source):
    ...
    pairs=[g for g in baseline['graphs'] if g['derivation_ids']==['000009']]
    require(len(pairs)==1,'One integration_parts source pair required');pair=pairs[0]
    ...
    with psycopg.connect(dotenv_values(ROOT/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row,
        ...
        for version,key in zip(versions,['legacy','projected']):
            ...
            require(row and row['artifact_id']==pair[key+'_artifact_id'] and row['content_hash']==pair[key+'_content_hash'],'Original integration_parts identity changed')
        ...
        a,b=forms
        require(signature(a[0])==signature(b[0]) and a[2]==b[2] and {k:Counter(v) for k,v in a[1].items()}=={k:Counter(v) for k,v in b[1].items()},'Complete integration_parts grouped scope differs')
        require(inventories[0]['relations']==inventories[1]['relations'] and inventories[0]['binding_versions']==inventories[1]['binding_versions']
            and all(i['all_bindings_active'] for i in inventories),'Integration by parts inference/binding contracts differ')
    reports=[]
    for key,row in zip(['legacy','projected'],sources):
        reports.append(dict(family='integration_parts_'+key,legacy_artifact_id=row['artifact_id'],legacy_version_id=row['version_id'],legacy_content_hash=row['content_hash'],
    ...
    return dict(read_only=True,approved=False,catalog_mutations=0,graphs=reports,fresh_execution=execution,
        auditor_sha256=sha(__file__),parent_checker_sha256=sha(ROOT/'scripts/check_integration_parts_identity_parent.py'),
        limitations=['Explicit parametrized differential reconstruction with independent integration constant; original incomplete AST remains provenance.',
            'Scalar commutative C1 functions on a common connected real interval established by the caller, including branch and pole exclusions.',
            'Portable symbolic integrand and antiderivative with residual integral retained; not numerical quadrature or automatic domain certification.'])
...
if __name__=='__main__':
    ...
    (ROOT/'docs/reviews/integration_parts_identity_execution_scope.json').write_text(json.dumps(report,indent=2)+'\n')
