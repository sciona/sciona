"""Activate the qualified original corrected quadratic revision at automated Tier 3."""
import argparse
import json
...
from scripts.review_quadratic_revision import review,LIMITATIONS
...
RUNNER='quadratic-original-revision-community.v1'
DESCRIPTION='Solve a real quadratic with both ordered roots using an explicitly corrected derivation and one approved reusable provider. Inputs are finite real coefficients a, b and c with identical nonempty shapes, nonzero a, nonnegative discriminant and consistent polynomial units. Preserves repeated and zero roots and source correction history. Automated Tier 3; no parity claim with the defective source.'
PLAIN_DESCRIPTION='Find both real solutions of a quadratic equation, returning the lower solution first. The same solver supports caller-defined applications such as motion and cost models. Supply coefficients in consistent units and decide which roots apply to your problem. Linear equations and complex solutions are outside its scope.'
REFERENCE='openstax-algebra-trigonometry-2e-2-5'
DOMAIN='Finite real coefficients in consistent units with identical nonempty shapes, a nonzero, discriminant nonnegative; both roots representable as finite float64, including exact zero roots. Physical applicability and admissible-root selection are caller responsibilities.'
...
def promote(source,apply=False):
    if apply:
        gates=json.loads((ROOT/'docs/reviews/quadratic_revision_publication_transaction.json').read_text())
        require(gates['passed'] and gates['post_activation_failure_rolled_back']
            and gates['publisher_sha256']==sha(__file__)
            and gates['validator_sha256']==sha(ROOT/'scripts/validate_quadratic_publication_transaction.py'),'Publication rollback qualification required')
    ...
    with psycopg.connect(dotenv_values(ROOT/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row,
        ...
        db.execute("SELECT pg_advisory_xact_lock(hashtext('quadratic-original-revision-draft.v1'))")
        ...
        bound=uuid5(version,RUNNER+':real-quadratic-regime')
        ...
        if not already:
            updated+=db.execute("UPDATE artifacts SET status='approved',is_publishable=true,description=%s,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=3,top_level_output_arity=2,topo_hash=%s WHERE artifact_id=%s",(DESCRIPTION,topology,target)).rowcount
...
if __name__=='__main__':
    ...
    if args.apply:
        name='quadratic_revision_publication_repeat.json' if report['already_approved'] else 'quadratic_revision_publication.json'
