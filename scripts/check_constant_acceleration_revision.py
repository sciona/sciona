...
from scripts.stage_constant_acceleration_original_revision import ROOT,ARTIFACT,ORIGINAL,EXECUTION,history,sha,require
from sciona.atoms.physics.constant_acceleration import constant_acceleration
from sciona.services.catalog_artifact_retrieval import _artifact_document_to_cdg
from sciona.services.execution_graph_codec import encode_execution_graph


def check(db,approved=False):
    imported=json.loads((ROOT/'docs/reviews/constant_acceleration_original_revision_import.json').read_text())
    require(imported['stager_sha256']==sha(ROOT/'scripts/stage_constant_acceleration_original_revision.py'),'Stager changed')
    require(imported['scope_audit_sha256']==sha(ROOT/'docs/reviews/physics_constant_acceleration_original_scope.json'),'Scope evidence changed')
    ...
    require(selected==[dict(version_id=version,content_hash=digest,is_latest=approved,trust_tier=3,derives_from=ORIGINAL)],'Revision version differs')
    ...
    require(len(bindings)==1 and bindings[0]['node_id']=='motion' and bindings[0]['status']=='active'
    ...
    approvals=db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='constant_acceleration-corrected-community.v1' AND passed",(EXECUTION,)).fetchall()
    require(len(approvals)==1,'Parent semantic approval missing')
    evidence=ROOT/'docs/reviews/constant_acceleration_execution.json'
    require(sha(evidence)==approvals[0]['details']['evidence_sha256']['constant_acceleration_execution.json'],'Parent evidence changed')
    qualified=json.loads(evidence.read_text())
    require(sha(inspect.getfile(constant_acceleration))==qualified['provider_sha256'],'Provider source changed')
    require(list(inspect.signature(constant_acceleration).parameters)==['initial_velocity','acceleration','elapsed_time'],'Provider signature differs')
    ...
    expected_deps=[]
    for source in [ORIGINAL,EXECUTION]:
