"""Verify full staging rollback and failure near the end of first-wave staging."""
import argparse
import hashlib
import json
...
from scripts.stage_first_wave_identity_revisions import ROOT,stage,sha,require


def snapshot():
    scope=json.loads((ROOT/'docs/reviews/first_wave_identity_execution_scope.json').read_text())
...
def validate(source):
    ...
    require(len(dry['graphs'])==1 and all(r['rows_created']>0 for r in dry['graphs']),'Absent-candidate baseline required')
    require(snapshot()==before,'Full staging rollback changed catalog')
    failure=dry['graphs'][0]['rows_created']-1
    ...
    require(snapshot()==before,'Partial-family rollback changed catalog')
    return dict(passed=True,rollback_verified=True,injected_failure_rollback_verified=True,
        injected_after_actual_writes=failure,completed_families_before_injected_failure=0,committed_catalog_mutations=0,
        draft=dry,stager_sha256=sha(ROOT/'scripts/stage_first_wave_identity_revisions.py'),validator_sha256=sha(__file__))
...
if __name__=='__main__':
    ...
    (ROOT/'docs/reviews/first_wave_identity_revision_transaction.json').write_text(json.dumps(report,indent=2)+'\n')
    print(json.dumps(dict(passed=True,graphs=1,committed_catalog_mutations=0,injected_after_actual_writes=report['injected_after_actual_writes'])))
