...
from scripts.stage_parallel_resistance_original_revision import ROOT,ARTIFACT,ORIGINAL,EXECUTION,history,sha,require
...
def check(db,approved=False):
    imported=json.loads((ROOT/'docs/reviews/parallel_resistance_original_revision_import.json').read_text())
    require(imported['stager_sha256']==sha(ROOT/'scripts/stage_parallel_resistance_original_revision.py'),'Stager changed')
    require(imported['scope_audit_sha256']==sha(ROOT/'docs/reviews/physics_parallel_resistance_original_scope.json'),'Scope evidence changed')
    ...
    require(len(bindings)==1 and bindings[0]['node_id']=='network' and bindings[0]['status']=='active'
    ...
    approvals=db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='parallel_resistance-corrected-community.v1' AND passed",(EXECUTION,)).fetchall()
    require(len(approvals)==1,'Parent semantic approval missing')
    evidence=ROOT/'docs/reviews/parallel_resistance_execution.json'
    require(sha(evidence)==approvals[0]['details']['evidence_sha256']['parallel_resistance_execution.json'],'Parent evidence changed')
    qualified=json.loads(evidence.read_text())
    require(sha(inspect.getfile(parallel_resistance))==qualified['provider_sha256'],'Provider source changed')
    require(list(inspect.signature(parallel_resistance).parameters)==['resistance_1','resistance_2','voltage'],'Provider signature differs')
