...
import scripts.promote_conditional_correction as publisher
...
def check_rollback(proposed):
    with psycopg.connect(dotenv_values(ROOT / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        require(not db.execute("SELECT 1 FROM references_registry WHERE ref_id='conditional-correction-drivendata-source'").fetchone(), 'Reference survived rollback')
...
if __name__ == '__main__':
    report = validate()
    (ROOT / 'docs/reviews/conditional_correction_publication_transaction_gates.json').write_text(json.dumps(report, indent=2) + '\n')
