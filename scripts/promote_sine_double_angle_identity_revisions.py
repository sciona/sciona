"""Atomically activate both qualified sine_double_angle revisions at automated Tier 3."""
import argparse
import importlib
import json
...
from scripts.stage_sine_double_angle_identity_revisions import ROOT,sha,require,RUNNER as DRAFT_RUNNER
from scripts.review_sine_double_angle_identity_publication import review
...
RUNNER='sine_double_angle-original-identities-community.v1'


def publication_fields(db,item):
    fields=item['publication']
    return fields['technical'],fields['plain'],fields['bound'],fields['reference']
...
def promote(source,apply=False):
    if apply:
        gates=json.loads((ROOT/'docs/reviews/sine_double_angle_identity_publication_transaction.json').read_text())
        require(gates['passed'] and gates['post_activation_failure_rolled_back'] and gates['publisher_sha256']==sha(__file__)
            and gates['validator_sha256']==sha(ROOT/'scripts/validate_sine_double_angle_identity_publication_transaction.py'),'Qualified publication rollback required')
...
if __name__=='__main__':
    ...
    if args.apply:
        name='sine_double_angle_identity_publication_repeat.json' if report['already_approved'] else 'sine_double_angle_identity_publication.json'
