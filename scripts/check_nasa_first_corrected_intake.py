...
PUBLICATION_RUNNER="nasa-first-corrected-intake-community.v1"
...
def check_candidate(db,proposed,*,approved=False):
    ...
    require(state==dict(status='approved' if approved else 'draft',is_publishable=approved,content_hash=proposed['graph_sha256'],is_latest=approved,trust_tier=3),'Candidate selection state differs')
    require(db.execute('SELECT is_latest FROM artifact_versions WHERE version_id=%s',(proposed['original_version_id'],)).fetchone()['is_latest']==(not approved),'Original latest version changed')
    ...
    require(bool(db.execute('SELECT 1 FROM catalog_artifacts_served WHERE artifact_id=%s',(proposed['artifact_id'],)).fetchone())==approved,'Serving state differs')
    return graph
