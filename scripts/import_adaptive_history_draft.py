"""Transactionally stage reviewed adaptive-history artifacts; default is rollback."""
import argparse
import json
...
from scripts.review_conditional_correction import ROOT, SOURCE_VERSION, SOURCE_HASH, require
from scripts.review_adaptive_history import review, FQDN

RUNNER = 'adaptive-history-draft.v1'


def audit():
    result = review()
    # Identity observations are mutable catalog state, not immutable semantics.
    for identity in result['identities']:
        identity.pop('exists', None)
    result['scope'] = 'One-shot adaptive lookback count, mean and population standard deviation with explicit controls and missing statistics.'
    return result


def plan():
    semantic = audit()
    graph = build_adaptive_history_graph()
    node = graph.nodes[0]
    graph_identity = next(i for i in semantic['identities'] if i['fqdn'] == FQDN)
    atom_identity = next(i for i in semantic['identities'] if i['fqdn'] != FQDN)
    return dict(fqdn=FQDN, artifact_id=graph_identity['artifact_id'], version_id=semantic['graph_version_id'],
        graph_sha256=semantic['graph_sha256'],
        bindings=[dict(node_id=node.node_id, fqdn=atom_identity['fqdn'], artifact_id=atom_identity['artifact_id'],
            version_id=semantic['provider_version_id'], content_hash=semantic['provider_content_hash'],
            runtime_fqdn=node.matched_primitive, inputs=[p.model_dump() for p in node.inputs], outputs=[p.model_dump() for p in node.outputs])],
        boundary_inputs=[p.model_dump() for p in node.inputs], boundary_outputs=[p.model_dump() for p in node.outputs],
        mandatory_provenance=dict(fqdn='cdg.competition.solution.kaggle.nasa_airport_pushback_phase_1_3rd_place_1st',
            scope='Mandatory original intake provenance; adaptive statistics subgraph only, not an invoked numerical dependency.'))
...
def stage(apply=False):
    ...
    with psycopg.connect(dotenv_values(ROOT / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if not apply:
            db.rollback()
    return dict(applied=apply, rows_created=created, draft_atoms=1, draft_cdgs=1, approved=False,
