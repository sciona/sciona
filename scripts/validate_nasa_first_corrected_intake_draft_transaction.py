...
import scripts.import_nasa_first_corrected_intake as importer
from scripts.plan_nasa_first_corrected_intake import ROOT,plan,origin_rows,check_source_anchor,require,sha
...
def validate():
    ...
    original=importer.ensure_row;checked=[]
    for stop in [3,rollback['rows_created']//2,rollback['rows_created']]:
        writes=0
        def fault(*args,**kwargs):
            nonlocal writes
            result=original(*args,**kwargs);writes+=int(result)
            if writes==stop:raise RuntimeError('injected corrected-intake failure')
            return result
        ...
        require(writes==stop,'Failure must follow actual writes');unchanged(proposed);checked.append(stop)
    return dict(passed=True,rollback=rollback,failure_rollback_verified=True,injected_after_actual_writes=checked,
        committed_catalog_mutations=0,importer_sha256=sha(Path(importer.__file__)),validator_sha256=sha(Path(__file__)))


if __name__=='__main__':
    report=validate()
    (ROOT/'docs/reviews/competition_nasa_first_corrected_intake_draft_transaction.json').write_text(json.dumps(report,indent=2)+'\n')
