"""Activate the qualified original reconstructed projectile revision at automated Tier 3."""
import argparse
import json
...
from scripts.review_projectile_revision import review,LIMITATIONS
...
RUNNER='projectile-original-revision-community.v1'
DESCRIPTION='Evaluate corrected ideal-projectile elapsed time and height at a horizontal coordinate. Reuses one approved provider with exact converted-input arithmetic before independent float64 rounding. Six SI input arrays have identical nonempty shapes; nonzero horizontal velocity, nonnegative gravity and forward time are required. Automated Tier 3; recovered source equations and symbolic corrections remain explicit.'
PLAIN_DESCRIPTION='Calculate elapsed time and height at a chosen horizontal position for ideal motion without drag. Supply the initial position, both initial velocity components and constant downward gravity in SI units. Either horizontal direction is supported. This component does not detect ground impacts or model wind or varying gravity.'
REFERENCE='openstax-university-physics-v1-4-3'
DOMAIN='Ideal no-drag constant-acceleration motion in one Cartesian frame. Finite real SI inputs with identical nonempty shapes, nonzero horizontal velocity, nonnegative downward gravity and nonnegative elapsed time. Exact converted-input time is used before independent output rounding. No collision or terrain cutoff.'
...
def promote(source,apply=False):
    if apply:
        gates=json.loads((ROOT/'docs/reviews/projectile_revision_publication_transaction.json').read_text())
        require(gates['passed'] and gates['post_activation_failure_rolled_back']
            and gates['publisher_sha256']==sha(__file__)
            and gates['validator_sha256']==sha(ROOT/'scripts/validate_projectile_publication_transaction.py'),'Publication rollback qualification required')
    ...
    with psycopg.connect(dotenv_values(ROOT/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row,
        ...
        bound=uuid5(version,RUNNER+':ideal-projectile-regime')
        ...
        if not already:
            updated+=db.execute("UPDATE artifacts SET status='approved',is_publishable=true,description=%s,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=6,top_level_output_arity=2,topo_hash=%s WHERE artifact_id=%s",(DESCRIPTION,topology,target)).rowcount
...
if __name__=='__main__':
    ...
    if args.apply:
        name='projectile_revision_publication_repeat.json' if report['already_approved'] else 'projectile_revision_publication.json'
