"""Stage both original sine_double_angle identities with their qualified real-radian realization."""
import argparse
import hashlib
import importlib
import json
...
from scripts.audit_sine_double_angle_identity_scope import ROOT,audit,require,check_parent
from scripts.import_residual_execution_drafts import ensure_row
from sciona.services.execution_graph_codec import encode_execution_graph
from sciona.services.catalog_artifact_retrieval import _artifact_document_to_cdg

RUNNER='sine_double_angle-original-identities-draft.v1'
SCOPE='Reviewed sine double-angle identity with explicit imaginary-unit source interpretation, realized for finite real radians; original source classifications remain immutable provenance.'
...
def stage(source,apply=False,fail_after_writes=None):
    if apply:
        gates=json.loads((ROOT/'docs/reviews/sine_double_angle_identity_revision_transaction.json').read_text())
        require(gates['passed'] and gates['rollback_verified'] and gates['injected_failure_rollback_verified']
            and gates['stager_sha256']==sha(__file__) and gates['validator_sha256']==sha(ROOT/'scripts/validate_sine_double_angle_identity_transaction.py'),'Current transaction qualification required')
    scope=audit(source)
    require(scope==json.loads((ROOT/'docs/reviews/sine_double_angle_identity_execution_scope.json').read_text()),'Qualified legacy scopes changed')
    created=0;reports=[]
        ...
        for item in sorted(scope['graphs'],key=lambda r:r['legacy_artifact_id']):
            ...
            for direction,ports in [('input',graph.nodes[0].inputs),('output',graph.nodes[0].outputs)]:
                ...
                binding_confidence=1.,binding_source=RUNNER,alternatives=Jsonb([]),evidence_summary=Jsonb(dict(
                    qualified_parent_evidence=parent_evidence,qualified_parent_version_id=qualified['version_id'],legacy_scope_sha256=sha(ROOT/'docs/reviews/sine_double_angle_identity_execution_scope.json')))))
    ...
    return dict(applied=apply,approved=False,rows_created=created,graphs=reports,stager_sha256=sha(__file__),
        scope_audit_sha256=sha(ROOT/'docs/reviews/sine_double_angle_identity_execution_scope.json'))
...
if __name__=='__main__':
    ...
    if args.apply:
        name='sine_double_angle_identity_revision_repeat.json' if report['rows_created']==0 else 'sine_double_angle_identity_revision_import.json'
