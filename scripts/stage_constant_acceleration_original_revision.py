"""Stage a version of the original constant_acceleration artifact using its qualified execution graph."""
import argparse
import hashlib
import json
...
from scripts.audit_constant_acceleration_original_scope import ROOT,ARTIFACT,ORIGINAL,EXECUTION,audit,require
from scripts.import_residual_execution_drafts import ensure_row
from sciona.services.catalog_artifact_retrieval import _artifact_document_to_cdg
from sciona.services.execution_graph_codec import encode_execution_graph

RUNNER='constant_acceleration-original-revision-draft.v1'
...
def history(db):
    ...
    for table in ['artifact_versions','artifact_cdg_nodes','artifact_cdg_edges','artifact_cdg_bindings',
        ...
        rows=db.execute(f"SELECT to_jsonb(t)-'updated_at'-'is_latest' AS row FROM {table} t WHERE {key}=ANY(%s::uuid[]) ORDER BY (to_jsonb(t)-'updated_at'-'is_latest')::text",([ORIGINAL],)).fetchall()
...
def stage(source,apply=False,fail_after_writes=None):
    if apply:
        gates=json.loads((ROOT/'docs/reviews/constant_acceleration_original_revision_transaction.json').read_text())
        require(gates['passed'] and gates['rollback_verified'] and gates['injected_failure_rollback_verified']
            ...
            and gates['validator_sha256']==sha(ROOT/'scripts/validate_constant_acceleration_revision_transaction.py'),'Current draft transaction gates required')
    scope=audit(source)
    require(scope==json.loads((ROOT/'docs/reviews/physics_constant_acceleration_original_scope.json').read_text()),'Frozen scope evidence differs')
    created=0
    with psycopg.connect(dotenv_values(ROOT/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row,
        ...
        require(digest==scope['approved_execution_graph_sha256'] and len(nodes)==1 and not edges,'Qualified graph differs')
        ...
        graph.metadata.update(original_artifact_id=ARTIFACT,original_version_id=ORIGINAL,
            ...
            revision_scope='Corrected complete signed constant-acceleration motion under original identity; missing equation, damaged squared-velocity AST and quotient domains remain explicit immutable provenance; no literal source-proof parity claim.')
        digest,nodes,edges=encode_execution_graph(graph)
        ...
        def ensure(table,keys,row):
            ...
            content_hash=digest,semver='0.0.0+execution.'+digest[:12],is_latest=False,trust_tier=3,derives_from=UUID(ORIGINAL)))
        for node in nodes:
        ...
        ensure('artifact_cdg_bindings',dict(version_id=version,node_id='motion'),dict(version_id=version,node_id='motion',
            **binding,binding_confidence=1.0,binding_source=RUNNER,alternatives=Jsonb([]),
            evidence_summary=Jsonb(dict(provider_version_id=str(providers[0]['version_id']),scope_audit_sha256=sha(ROOT/'docs/reviews/physics_constant_acceleration_original_scope.json')))))
        for selected in [ORIGINAL,EXECUTION]:
    ...
    return dict(applied=apply,approved=False,rows_created=created,artifact_id=ARTIFACT,version_id=str(version),
        ...
        stager_sha256=sha(__file__),scope_audit_sha256=sha(ROOT/'docs/reviews/physics_constant_acceleration_original_scope.json'))
...
if __name__=='__main__':
    ...
    if args.apply:
        name='constant_acceleration_original_revision_repeat.json' if report['rows_created']==0 else 'constant_acceleration_original_revision_import.json'
