...
from scripts.stage_remaining_legacy_physics_revisions import ROOT,sha,require,RUNNER as DRAFT_RUNNER
...
RUNNER='remaining-legacy-physics-community.v1'
...
def promote(source,apply=False):
    if apply:
        gates=json.loads((ROOT/'docs/reviews/remaining_legacy_physics_publication_transaction.json').read_text())
        require(gates['passed'] and gates['post_activation_failure_rolled_back'] and gates['publisher_sha256']==sha(__file__)
            and gates['validator_sha256']==sha(ROOT/'scripts/validate_remaining_legacy_physics_publication_transaction.py'),'Qualified publication rollback required')
...
if __name__=='__main__':
    ...
    if args.apply:
        name='remaining_legacy_physics_publication_repeat.json' if report['already_approved'] else 'remaining_legacy_physics_publication.json'
