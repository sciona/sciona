...
from scripts.stage_constant_acceleration_original_revision import ROOT,ARTIFACT,sha,require
from scripts.promote_constant_acceleration_revision import check_publication,DESCRIPTION,PLAIN_DESCRIPTION
from scripts.review_constant_acceleration_revision import review
from scripts.validate_constant_acceleration_revision_execution import validate
...
async def verify(source):
    ...
    return dict(passed=True,approved=True,trust_tier=3,selection=before,execution=execution,catalog_mutations=0,
        validator_sha256=sha(__file__),execution_validator_sha256=sha(ROOT/'scripts/validate_constant_acceleration_revision_execution.py'),
...
if __name__=='__main__':
    ...
    (ROOT/'docs/reviews/constant_acceleration_revision_served_verification.json').write_text(json.dumps(report,indent=2)+'\n')
    print(json.dumps(dict(passed=True,approved=True,trust_tier=3,stored_states=29,maximum_ulp_error=0)))
