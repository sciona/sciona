"""Transactionally stage reviewed correction artifacts; default is rollback."""
import argparse
import json
...
from scripts.review_conditional_correction import ROOT, SOURCE_VERSION, SOURCE_HASH, audit, require

RUNNER = 'conditional-correction-draft.v1'
...
def stage(apply=False):
    ...
    with psycopg.connect(dotenv_values(ROOT / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        source = db.execute('SELECT a.status,a.is_publishable,v.content_hash FROM artifacts a JOIN artifact_versions v '
        ...
        if not apply:
            db.rollback()
    return dict(applied=apply, rows_created=created, draft_atoms=2, draft_cdgs=1, approved=False,
