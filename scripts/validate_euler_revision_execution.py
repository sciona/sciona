"""Execute the staged original-identity fixed proof graph using the existing exact certificate verifier."""
import argparse
import asyncio
import json
...
from scripts.stage_euler_original_revision import ROOT,ARTIFACT,ORIGINAL,EXECUTION,history,sha,require
...
import scripts.validate_euler_execution as numerical
...
def stored_graph(imported):
    ...
    expected=build_euler_execution().model_copy(deep=True)
    expected.metadata.update(original_artifact_id=ARTIFACT,original_version_id=ORIGINAL,qualified_numerical_parent_version_id=EXECUTION,
        revision_scope='Fixed exact Euler proof under reviewed constant interpretation; original and replay derivations remain immutable provenance; no literal source-parity claim.')
    require(expected==graph and encode_execution_graph(graph)[0]==imported['graph_sha256'],'Stored graph differs from qualified proof evaluator')
    return graph


async def validate(source):
    imported=json.loads((ROOT/'docs/reviews/euler_original_revision_import.json').read_text())
    require(imported['stager_sha256']==sha(ROOT/'scripts/stage_euler_original_revision.py'),'Stager code changed')
    scope=audit(source)
    require(scope==json.loads((ROOT/'docs/reviews/physics_euler_original_scope.json').read_text())
        and imported['scope_audit_sha256']==sha(ROOT/'docs/reviews/physics_euler_original_scope.json'),'Source qualification changed')
    graph=stored_graph(imported)
    # Substitute the explicitly selected stored graph at the numerical runner's
    # construction boundary. Source interpretation checks and exact certificate comparisons run unchanged.
    with patch.object(numerical,'build_euler_execution',return_value=graph):
        execution=await numerical.validate(ROOT,source/'symbols.cypher',source/'infrules.cypher')
    require(execution['graph_digest']==imported['graph_sha256'] and execution['full_runner_cases']==2
        and execution['proof_steps_per_run']==4 and execution['exact_source_interpreted_step_parity'],'Stored exact certificate qualification differs')
    ...
    return dict(passed=True,approved=False,catalog_mutations=0,version_id=imported['version_id'],graph_sha256=imported['graph_sha256'],
        ...
        scope_audit_sha256=imported['scope_audit_sha256'],limitations=[
            ...
            'No original artifact approval, literal source parity, general theorem-proving or physical simulation claim.'])
...
if __name__=='__main__':
    ...
    (ROOT/'docs/reviews/euler_revision_catalog_execution.json').write_text(json.dumps(report,indent=2)+'\n')
    print(json.dumps(dict(passed=True,approved=False,fresh_runs=2,proof_steps_per_run=4)))
