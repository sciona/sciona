"""Atomically stage non-publishable reusable providers; default is rollback."""
import argparse
import json
...
from scripts.plan_available_feature_providers import ROOT,MODULE,plan,evidence

RUNNER='available-feature-provider-draft.v1'
...
def stage(apply=False,fault_after=None):
    ...
    specs={spec.fqdn:spec for spec in _parse_registered_atoms(repo=ProviderRepo('sciona-atoms-ml',repo),
        artifact_root=repo/'src/sciona/atoms') if spec.import_module==MODULE}
    created=0
