"""Atomically approve the qualified adaptive-history drafts at automated Tier 3."""
import argparse
import json
...
from scripts.review_adaptive_history_publication import review, LIMITATIONS
from scripts.validate_adaptive_history_database_gates import check_staged

RUNNER = 'adaptive-history-community.v1'


def promote(apply=False):
    if apply:
        gate_path = ROOT / 'docs/reviews/adaptive_history_publication_transaction_gates.json'
        gates = json.loads(gate_path.read_text())
        require(gates['passed'] is True and gates['publisher_sha256'] == sha(Path(__file__))
                and gates['validator_sha256'] == sha(ROOT / 'scripts/validate_adaptive_history_publication_transaction.py')
    ...
    with psycopg.connect(dotenv_values(ROOT / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        require(len(states) == 2, 'Staged artifacts missing')
        ...
        for target, selected, binding in targets:
            ...
            details = dict(qualification=qualification, publisher_sha256=sha(Path(__file__)), publication_tier=3,
                           scope='Provisioned in-process one-shot adaptive history statistics', binding=binding)
        ...
        if not already:
            db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,"
                       'top_level_input_arity=5,top_level_output_arity=3,topo_hash=%s WHERE artifact_id=%s', (topology, identity))
    ...
    return dict(applied=apply, already_approved=already, rows_created=created, trust_tier=3,
                served_atoms_in_transaction=1, served_cdgs_in_transaction=1, version_id=str(version), graph_sha256=proposed['graph_sha256'])
