...
import importlib
import json
from scripts.stage_first_wave_identity_revisions import ROOT,history,revised_graph,sha,require
from sciona.services.execution_graph_codec import encode_execution_graph
from sciona.services.catalog_artifact_retrieval import _artifact_document_to_cdg


def check(db,family,approved=False):
    imported=json.loads((ROOT/'docs/reviews/first_wave_identity_revision_import.json').read_text())
    require(imported['stager_sha256']==sha(ROOT/'scripts/stage_first_wave_identity_revisions.py'),'Legacy stager changed')
    require(imported['scope_audit_sha256']==sha(ROOT/'docs/reviews/first_wave_identity_execution_scope.json'),'Legacy scope evidence changed')
    items=[r for r in imported['graphs'] if r['family']==family]
    scopes=[r for r in json.loads((ROOT/'docs/reviews/first_wave_identity_execution_scope.json').read_text())['graphs'] if r['family']==family]
    ...
    from scripts.audit_first_wave_revision_scope import check_parent
    require(sha(ROOT/'scripts/audit_first_wave_revision_scope.py')==json.loads((ROOT/'docs/reviews/first_wave_identity_execution_scope.json').read_text())['auditor_sha256'],'Scope auditor changed')
    require(sha(ROOT/'scripts/check_first_wave_parent_database.py')==json.loads((ROOT/'docs/reviews/first_wave_identity_execution_scope.json').read_text())['parent_checker_sha256'],'Parent checker changed')
    ...
    deps=[]
    for selected in [item['original_version_id'],qualified['version_id']]:
        ...
        deps.append(dict(dependency_artifact_fqdn=target['fqdn'],dependency_content_hash=target['content_hash'],dependency_role='cdg',optional=False,port_name='',
            binding_metadata=dict(source_version_id=selected,scope='Immutable first-wave source and approved corrected dynamics provenance; runtime uses direct provider binding.')))
