...
from scripts.stage_euler_original_revision import ROOT,ARTIFACT,sha,require
from scripts.promote_euler_revision import check_publication,DESCRIPTION,PLAIN_DESCRIPTION
from scripts.review_euler_revision import review
...
async def verify(source):
    ...
    # Source interpretation and exact certificate checks remain unchanged; no database state is mocked.
    ...
    require(execution['graph_digest']==imported['graph_sha256'] and execution['full_runner_cases']==2
        and execution['proof_steps_per_run']==4 and execution['exact_source_interpreted_step_parity'],'Fresh stored exact proof execution differs')
    ...
    return dict(passed=True,approved=True,trust_tier=3,selection=before,execution=execution,catalog_mutations=0,
        validator_sha256=sha(__file__),execution_validator_sha256=sha(ROOT/'scripts/validate_euler_execution.py'),
...
if __name__=='__main__':
    ...
    (ROOT/'docs/reviews/euler_revision_served_verification.json').write_text(json.dumps(report,indent=2)+'\n')
    print(json.dumps(dict(passed=True,approved=True,trust_tier=3,fresh_runs=2,proof_steps_per_run=4)))
