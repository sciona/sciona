...
from scripts.stage_first_wave_identity_revisions import ROOT,sha,require,RUNNER
...
def cases(item):
    ...
    return [
        ('parent_binding_hash','UPDATE artifact_cdg_bindings SET bound_version_content_hash=%s WHERE version_id=%s',('0'*64,parent),''),
        ('parent_approval',"UPDATE artifact_audit_evidence SET passed=false WHERE version_id=%s AND runner_version='first_wave_dynamics-execution-community.v1'",(parent,),''),
        ('parent_provenance','UPDATE artifact_dependencies SET optional=true WHERE dependent_version_id=%s',(parent,),''),
...
def validate():
    items=json.loads((ROOT/'docs/reviews/first_wave_identity_revision_import.json').read_text())['graphs'];reports=[]
        ...
        for item in items:
            ...
            for name,statement,params,expected in cases(item):
                ...
                try:
                    ...
                    try:check(db,family)
                    except (ValueError,AssertionError) as error:require(str(error)==expected,'Unexpected '+family+'/'+name+' rejection: '+str(error));rejected.append(name)
    ...
    return dict(passed=True,approved=False,rollback_verified=True,committed_catalog_mutations=0,graphs=reports,
        validator_sha256=sha(__file__),checker_sha256=sha(ROOT/'scripts/check_first_wave_identity_revision.py'),
        scope='Fourteen candidate faults and three qualified-parent faults in the same transaction, each followed by intact reads; final connection verifies rollback.')


if __name__=='__main__':
    report=validate()
    (ROOT/'docs/reviews/first_wave_identity_database_gates.json').write_text(json.dumps(report,indent=2)+'\n')
    print(json.dumps(dict(passed=True,graphs=1,rejected_faults=sum(len(r['rejected_faults']) for r in report['graphs']),committed_catalog_mutations=0)))
