"""Run actual stored projectile revision with immutable source and Decimal oracle."""
import argparse
import asyncio
import json
...
from scripts.stage_projectile_original_revision import ROOT,sha,require
...
async def validate(source,approved=False):
    scope=audit(source)
    require(scope==json.loads((ROOT/'docs/reviews/physics_projectile_original_scope.json').read_text()),'Qualified scope changed')
    ...
    with patch.object(oracle,'build_projectile_execution',return_value=graph),patch.object(oracle,'validate_source',side_effect=validate_source):
    ...
    require(execution['graph_digest']==imported['graph_sha256'] and execution['full_runner_cases']==6
        and execution['synthetic_points']==29 and execution['maximum_ulp_error']==0,'Stored execution coverage differs')
    ...
    return dict(passed=True,approved=approved,catalog_mutations=0,version_id=imported['version_id'],graph_sha256=imported['graph_sha256'],
        original_history_preserved=True,stored_execution=execution,validator_sha256=sha(__file__),checker_sha256=sha(ROOT/'scripts/check_projectile_revision.py'),
        source_validator_sha256=sha(ROOT/'scripts/validate_projectile_immutable_source.py'),scope_audit_sha256=imported['scope_audit_sha256'],
        limitations=['Corrected ideal-projectile proof with explicit no-drag, constant-gravity premises; no literal source-rule parity, terrain or collision cutoff.',
...
if __name__=='__main__':
    ...
    name='projectile_revision_served_execution.json' if args.approved else 'projectile_revision_catalog_execution.json'
    (ROOT/'docs/reviews'/name).write_text(json.dumps(report,indent=2)+'\n')
    print(json.dumps(dict(passed=True,approved=args.approved,stored_points=29,maximum_ulp_error=0)))
