...
from scripts.stage_schwarzschild_original_revision import ROOT,ARTIFACT,sha,require
from scripts.promote_schwarzschild_revision import check_publication,DESCRIPTION,PLAIN_DESCRIPTION
from scripts.review_schwarzschild_revision import review
...
async def verify(source):
    ...
    require(execution['graph_digest']==imported['graph_sha256'] and execution['full_runner_cases']==5
        and execution['synthetic_numeric_cases']==208 and execution['maximum_ulp_error']<=3
        and execution['exact_rational_proof_cases']==3,'Fresh stored numerical execution differs')
    ...
    return dict(passed=True,approved=True,trust_tier=3,selection=before,execution=execution,catalog_mutations=0,
        validator_sha256=sha(__file__),execution_validator_sha256=sha(ROOT/'scripts/validate_schwarzschild_execution.py'),
...
if __name__=='__main__':
    ...
    (ROOT/'docs/reviews/schwarzschild_revision_served_verification.json').write_text(json.dumps(report,indent=2)+'\n')
