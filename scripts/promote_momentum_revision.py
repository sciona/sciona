"""Activate the qualified original reconstructed momentum revision at automated Tier 3."""
import argparse
import json
...
from scripts.review_momentum_revision import review,LIMITATIONS
...
RUNNER='momentum-original-revision-community.v1'
DESCRIPTION='Compute real 3D recoil momentum and its squared Euclidean norm from incoming and outgoing momentum vectors. Reuses one approved provider with exact converted-input arithmetic before independent float64 rounding. Inputs share an orthonormal frame, SI momentum units and identical nonempty shapes ending in three. Automated Tier 3 reconstructed source proof; original incomplete dot-product expression remains preserved.'
PLAIN_DESCRIPTION='Subtract outgoing momentum from incoming momentum and return both the recoil vector and its squared magnitude. Supply vectors in the same frame and units. Recoil interpretation assumes momentum conservation; this component does not determine energy, scattering angles or whether a physical event is valid.'
REFERENCE='openstax-university-physics-v3-6-3'
DOMAIN='Real finite momentum vectors in one orthonormal frame and consistent SI units, identical nonempty shapes (...,3). Recoil interpretation requires momentum-conservation premises. Exact converted-input differences are squared before independent output rounding; finite representable outputs required, with exact zeros and subnormals supported.'
...
def promote(source,apply=False):
    if apply:
        gates=json.loads((ROOT/'docs/reviews/momentum_revision_publication_transaction.json').read_text())
        require(gates['passed'] and gates['post_activation_failure_rolled_back']
            and gates['publisher_sha256']==sha(__file__)
            and gates['validator_sha256']==sha(ROOT/'scripts/validate_momentum_publication_transaction.py'),'Publication rollback qualification required')
    ...
    with psycopg.connect(dotenv_values(ROOT/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row,
        ...
        db.execute("SELECT pg_advisory_xact_lock(hashtext('momentum-original-revision-draft.v1'))")
        ...
        bound=uuid5(version,RUNNER+':real-vector-recoil-regime')
        ...
        if not already:
            updated+=db.execute("UPDATE artifacts SET status='approved',is_publishable=true,description=%s,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=2,top_level_output_arity=2,topo_hash=%s WHERE artifact_id=%s",(DESCRIPTION,topology,target)).rowcount
...
if __name__=='__main__':
    ...
    if args.apply:
        name='momentum_revision_publication_repeat.json' if report['already_approved'] else 'momentum_revision_publication.json'
