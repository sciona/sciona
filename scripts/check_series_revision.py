...
from scripts.stage_series_original_revision import ROOT,ARTIFACT,ORIGINAL,EXECUTION,history,sha,require
...
def check(db,approved=False):
    imported=json.loads((ROOT/'docs/reviews/series_original_revision_import.json').read_text())
    require(imported['stager_sha256']==sha(ROOT/'scripts/stage_series_original_revision.py'),'Stager changed')
    require(imported['scope_audit_sha256']==sha(ROOT/'docs/reviews/physics_series_original_scope.json'),'Scope evidence changed')
    ...
    require(len(bindings)==1 and bindings[0]['node_id']=='series_equivalent_resistance' and bindings[0]['status']=='active'
    ...
    executed=db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='series-cdg-execution.v1' AND passed",(ORIGINAL,)).fetchall()
    require(len(executed)==1 and sha(inspect.getfile(series_resistance))==executed[0]['details']['provider_implementation_sha256'],'Provider source changed')
    require(list(inspect.signature(series_resistance).parameters)==['resistance_a','resistance_b','current'],'Provider signature differs')
