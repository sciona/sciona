...
from scripts.stage_remaining_legacy_physics_revisions import ROOT,stage,sha,require


def snapshot():
    scope=json.loads((ROOT/'docs/reviews/remaining_legacy_physics_execution_scope.json').read_text())
...
def validate(source):
    ...
    require(len(dry['graphs'])==5 and all(r['rows_created']>0 for r in dry['graphs']),'Absent-candidate baseline required')
    ...
    return dict(passed=True,rollback_verified=True,injected_failure_rollback_verified=True,
        ...
        draft=dry,stager_sha256=sha(ROOT/'scripts/stage_remaining_legacy_physics_revisions.py'),validator_sha256=sha(__file__))
...
if __name__=='__main__':
    ...
    (ROOT/'docs/reviews/remaining_legacy_physics_revision_transaction.json').write_text(json.dumps(report,indent=2)+'\n')
    print(json.dumps(dict(passed=True,graphs=5,committed_catalog_mutations=0,injected_after_actual_writes=report['injected_after_actual_writes'])))
