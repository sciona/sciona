...
from scripts.review_nasa_first_corrected_publication import review, LIMITATIONS

RUNNER=PUBLICATION_RUNNER
DESCRIPTION=('Corrected ten-population native regression training and guarded single-population prediction with '
    'explicit raw feature preparation, residual/direct/global models, score acceptance, feature importance, '
    'portable policy-bound state and lazy fallback branches. Composes 47 approved dependencies in 343 outer nodes. '
    'Automated Tier 3 compositional synthetic qualification; empirical effectiveness remains unqualified.')
PLAIN_DESCRIPTION=('Train models for ten airport populations and predict for one selected population. '
    'Keep training-time feature choices with the saved models, preserve query order, and use explicit fallback '
    'predictions when needed. Reusable operations remain separate from airport-specific adapters. '
    'The corrected version preserves the original source history and failed preflight evidence.')


def promote(apply=False):
    if apply:
        gates=json.loads((ROOT/'docs/reviews/competition_nasa_first_corrected_publication_transaction_gates.json').read_text())
        require(gates['passed'] and gates['publisher_sha256']==sha(Path(__file__))
            and gates['validator_sha256']==sha(ROOT/'scripts/validate_nasa_first_corrected_publication_transaction.py')
    ...
    graph=build_nasa_first_complete_graph()
    ...
    with psycopg.connect(dotenv_values(ROOT/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row,
        ...
        for name in ['available-feature-provider-draft.v1','nasa-first-lifecycle-provider-draft.v1','nasa-first-corrected-intake-draft.v1']:
...
if __name__=='__main__':
    ...
    if report['applied']:
        name='competition_nasa_first_corrected_publication_repeat.json' if report['already_approved'] else 'competition_nasa_first_corrected_publication.json'
