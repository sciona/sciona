...
def main():
    ...
    try:
        import psycopg
        import scripts.validate_nasa_population_catalog as catalog
        if args.child_directory:
            snapshot=json.loads((args.child_directory/'catalog_inference.json').read_text())
            graph=catalog.materialize(snapshot)
            original_select=catalog.execution.select_population_graph
            def select(kind,payload):
                ...
                return graph
            with patch.object(catalog.execution,'select_population_graph',side_effect=select):
        ...
        def run(command,*positional,**keywords):
            selected=isinstance(command,list) and len(command)==4 and command[2]=='--child-directory' and Path(command[1]).name=='validate_nasa_population_catalog.py'
        ...
        (ROOT/'docs/reviews/nasa_population_runtime_profile.json').write_text(json.dumps(report,indent=2)+'\n')
