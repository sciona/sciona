...
from scripts.stage_schwarzschild_original_revision import ROOT,ARTIFACT,ORIGINAL,REPLAY,EXECUTION,history,sha,require
from sciona.atoms.physical_quantities.schwarzschild_radius import schwarzschild_radius
...
def check(db,approved=False):
    ...
    require(imported['stager_sha256']==sha(ROOT/'scripts/stage_schwarzschild_original_revision.py'),'Stager changed')
    require(imported['scope_audit_sha256']==sha(ROOT/'docs/reviews/physics_schwarzschild_original_scope.json'),'Scope evidence changed')
    ...
    require(len(bindings)==1 and bindings[0]['node_id']=='radius' and bindings[0]['status']=='active'
    ...
    approvals=db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='schwarzschild-community.v1' AND passed",(EXECUTION,)).fetchall()
    require(len(approvals)==1,'Parent semantic approval missing')
    evidence=ROOT/'docs/reviews/schwarzschild_execution.json'
    require(sha(evidence)==approvals[0]['details']['evidence_sha256']['schwarzschild_execution.json'],'Parent evidence changed')
    qualified=json.loads(evidence.read_text())
    require(sha(inspect.getfile(schwarzschild_radius))==qualified['provider_sha256'],'Provider source changed')
    require(list(inspect.signature(schwarzschild_radius).parameters)==['mass_kg','gravitational_constant','speed_of_light'],'Provider signature differs')
