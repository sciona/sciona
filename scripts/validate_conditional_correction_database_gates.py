...
def check_staged(db, proposed, *, approved=False):
    ...
    require(_artifact_document_to_cdg(document, version_id=proposed['version_id'], content_hash=proposed['graph_sha256'],
            require_execution_envelope=True) == build_conditional_correction_graph(), 'Stored graph differs')
...
def validate():
    ...
    faults = [
        ...
        ('graph_contract', "UPDATE artifact_io_specs SET constraints='' WHERE version_id=%s AND name='threshold'", (graph,), 'Stored port contracts differ'),
        ('legacy_contract', "UPDATE atom_io_specs SET constraints='' WHERE version_id=%s AND name='threshold'", (atom,), 'Stored port contracts differ'),
    ...
    with psycopg.connect(dotenv_values(ROOT / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        check_staged(db, proposed)
    return dict(format='conditional-correction-database-gates.v1', passed=True, rejected_faults=rejected,
...
if __name__ == '__main__':
    report = validate()
    (ROOT / 'docs/reviews/conditional_correction_database_gates.json').write_text(json.dumps(report, indent=2) + '\n')
