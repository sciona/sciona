"""Atomically approve the qualified nasa-population drafts at automated Tier 3."""
import argparse
import json
...
from scripts.plan_nasa_population_drafts import plan, BUILDERS
from scripts.review_conditional_correction import ROOT, require, sha
from scripts.review_nasa_population_publication import review, LIMITATIONS
from scripts.check_nasa_population_catalog import check_staged

RUNNER = 'nasa-population-community.v1'


def promote(apply=False):
    if apply:
        gate_path = ROOT / 'docs/reviews/nasa_population_publication_transaction_gates.json'
        gates = json.loads(gate_path.read_text())
        require(gates['passed'] is True and gates['publisher_sha256'] == sha(Path(__file__))
                and gates['validator_sha256'] == sha(ROOT / 'scripts/validate_nasa_population_publication_transaction.py')
    ...
    with psycopg.connect(dotenv_values(ROOT / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        db.execute("SELECT pg_advisory_xact_lock(hashtext('nasa-population-draft.v1'))")
        ...
        for target, selected, binding in targets:
            ...
            details = dict(qualification=qualification, publisher_sha256=sha(Path(__file__)), publication_tier=3,
                           scope='Provisioned in-process explicit population graph family with active-count selection and complete private runtime state; shared numerical and keyed routing atoms remain reusable across domains', binding=binding)
...
if __name__ == '__main__':
    ...
    if args.apply:
        report = 'nasa_population_publication_repeat.json' if result['already_approved'] else 'nasa_population_publication.json'
