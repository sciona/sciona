...
from scripts.promote_legacy_physics_revisions import check_publication,publication_fields
from scripts.validate_legacy_physics_execution import validate
...
async def verify(source):
    ...
    return dict(passed=True,approved=True,trust_tier=3,catalog_mutations=0,selection=before,execution=execution,
        ...
        validator_sha256=sha(__file__),execution_validator_sha256=sha(ROOT/'scripts/validate_legacy_physics_execution.py'),retriever_sha256=sha(ROOT/'sciona/services/catalog_artifact_retrieval.py'))
...
if __name__=='__main__':
    ...
    (ROOT/'docs/reviews/legacy_physics_served_verification.json').write_text(json.dumps(report,indent=2)+'\n')
