...
from scripts.stage_integration_parts_identity_revisions import ROOT,sha,require,RUNNER
...
def cases(item):
    ...
    return [
        ('parent_binding_hash','UPDATE artifact_cdg_bindings SET bound_version_content_hash=%s WHERE version_id=%s',('0'*64,parent),'Integration by parts parent binding differs'),
        ('parent_approval',"UPDATE artifact_audit_evidence SET passed=false WHERE version_id=%s AND runner_version='integration_parts-corrected-community.v1'",(parent,),'Unique approved integration_parts targets required'),
        ('parent_provenance','UPDATE artifact_dependencies SET optional=true WHERE dependent_version_id=%s',(parent,),'Integration by parts parent provenance differs'),
...
def validate():
    items=json.loads((ROOT/'docs/reviews/integration_parts_identity_revision_import.json').read_text())['graphs'];reports=[]
...
if __name__=='__main__':
    report=validate()
    (ROOT/'docs/reviews/integration_parts_identity_database_gates.json').write_text(json.dumps(report,indent=2)+'\n')
