...
import scripts.promote_nasa_population as publisher
from scripts.review_conditional_correction import ROOT, require, sha
from scripts.plan_nasa_population_drafts import plan
...
def check_rollback(proposed):
    with psycopg.connect(dotenv_values(ROOT / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        require(not db.execute("SELECT 1 FROM references_registry WHERE ref_id='nasa-population-drivendata-source'").fetchone(), 'Reference survived rollback')
...
if __name__ == '__main__':
    report = validate()
    (ROOT / 'docs/reviews/nasa_population_publication_transaction_gates.json').write_text(json.dumps(report, indent=2) + '\n')
