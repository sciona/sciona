...
from scripts.stage_euler_original_revision import ROOT,ARTIFACT,ORIGINAL,REPLAY,EXECUTION,history,sha,require
from sciona.atoms.physical_quantities.euler_identity_proof import euler_identity_proof
...
def check(db,approved=False):
    ...
    require(imported['stager_sha256']==sha(ROOT/'scripts/stage_euler_original_revision.py'),'Stager changed')
    require(imported['scope_audit_sha256']==sha(ROOT/'docs/reviews/physics_euler_original_scope.json'),'Scope evidence changed')
    ...
    require(len(bindings)==1 and bindings[0]['node_id']=='proof' and bindings[0]['status']=='active'
    ...
    approvals=db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='euler-interpreted-community.v1' AND passed",(EXECUTION,)).fetchall()
    require(len(approvals)==1,'Parent semantic approval missing')
    evidence=ROOT/'docs/reviews/euler_execution.json'
    require(sha(evidence)==approvals[0]['details']['evidence_sha256']['euler_execution.json'],'Parent evidence changed')
    qualified=json.loads(evidence.read_text())
    require(sha(inspect.getfile(euler_identity_proof))==qualified['provider_sha256'],'Provider source changed')
    require(list(inspect.signature(euler_identity_proof).parameters)==[],'Provider signature differs')
