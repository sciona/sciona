...
from scripts.audit_euler_original_scope import ROOT,ARTIFACT,ORIGINAL,REPLAY,EXECUTION,audit,require
from scripts.import_residual_execution_drafts import ensure_row
from sciona.services.catalog_artifact_retrieval import _artifact_document_to_cdg
from sciona.services.execution_graph_codec import encode_execution_graph

RUNNER='euler-original-revision-draft.v1'
...
def stage(source,apply=False,fail_after_writes=None):
    if apply:
        gates=json.loads((ROOT/'docs/reviews/euler_original_revision_transaction.json').read_text())
        require(gates['passed'] and gates['rollback_verified'] and gates['injected_failure_rollback_verified']
            ...
            and gates['validator_sha256']==sha(ROOT/'scripts/validate_euler_revision_transaction.py'),'Current draft transaction gates required')
    scope=audit(source)
    require(scope==json.loads((ROOT/'docs/reviews/physics_euler_original_scope.json').read_text()),'Frozen scope evidence differs')
    created=0
    with psycopg.connect(dotenv_values(ROOT/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row,
        ...
        ensure('artifact_cdg_bindings',dict(version_id=version,node_id='proof'),dict(version_id=version,node_id='proof',
            **binding,binding_confidence=1.0,binding_source=RUNNER,alternatives=Jsonb([]),
            evidence_summary=Jsonb(dict(provider_version_id=str(providers[0]['version_id']),scope_audit_sha256=sha(ROOT/'docs/reviews/physics_euler_original_scope.json')))))
    ...
    return dict(applied=apply,approved=False,rows_created=created,artifact_id=ARTIFACT,version_id=str(version),
        ...
        stager_sha256=sha(__file__),scope_audit_sha256=sha(ROOT/'docs/reviews/physics_euler_original_scope.json'))
...
if __name__=='__main__':
    ...
    if args.apply:
        name='euler_original_revision_repeat.json' if report['rows_created']==0 else 'euler_original_revision_import.json'
