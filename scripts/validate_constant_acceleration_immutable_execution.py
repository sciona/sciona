"""Requalify constant_acceleration execution using immutable source-version validation."""
import argparse
import asyncio
import json
from pathlib import Path
from unittest.mock import patch
from scripts.audit_constant_acceleration_original_scope import ROOT,audit,sha,require
...
async def validate(source):
    ...
    retained=json.loads((ROOT/'docs/reviews/constant_acceleration_execution.json').read_text())
    ...
    return dict(passed=True,approved_original=False,catalog_mutations=0,execution=result,scope_sha256=sha(ROOT/'docs/reviews/physics_constant_acceleration_original_scope.json'),validator_sha256=sha(__file__),
        source_validator_sha256=sha(ROOT/'scripts/validate_constant_acceleration_immutable_source.py'),
...
if __name__=='__main__':
    ...
    (ROOT/'docs/reviews/constant_acceleration_immutable_execution.json').write_text(json.dumps(report,indent=2)+'\n')
