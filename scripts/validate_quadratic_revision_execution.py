"""Execute the staged original-identity corrected quadratic graph using the existing precision oracle."""
import argparse
import asyncio
import json
...
from scripts.stage_quadratic_original_revision import ROOT,ARTIFACT,ORIGINAL,EXECUTION,history,sha,require
...
def stored_graph(imported):
    ...
    expected=build_quadratic_execution().model_copy(deep=True)
    expected.metadata.update(original_artifact_id=ARTIFACT,original_version_id=ORIGINAL,qualified_numerical_parent_version_id=EXECUTION,
        revision_scope='Explicitly corrected complete real quadratic solution under original identity; defective original and replay equations remain immutable provenance; no source-parity claim.')
...
async def validate(source):
    imported=json.loads((ROOT/'docs/reviews/quadratic_original_revision_import.json').read_text())
    require(imported['stager_sha256']==sha(ROOT/'scripts/stage_quadratic_original_revision.py'),'Stager code changed')
    scope=audit(source)
    require(scope==json.loads((ROOT/'docs/reviews/physics_quadratic_original_scope.json').read_text())
        and imported['scope_audit_sha256']==sha(ROOT/'docs/reviews/physics_quadratic_original_scope.json'),'Source qualification changed')
    ...
    require(execution['graph_digest']==imported['graph_sha256'] and execution['full_runner_cases']==5
        and execution['synthetic_polynomials']==305 and execution['maximum_ulp_error']<=3,'Stored precision qualification differs')
    ...
    return dict(passed=True,approved=False,catalog_mutations=0,version_id=imported['version_id'],graph_sha256=imported['graph_sha256'],
        ...
        scope_audit_sha256=imported['scope_audit_sha256'],limitations=[
            ...
            'No original artifact approval or claim of parity with the defective source; physical applicability and cross-domain reuse evidence remain separate requirements.'])
...
if __name__=='__main__':
    ...
    (ROOT/'docs/reviews/quadratic_revision_catalog_execution.json').write_text(json.dumps(report,indent=2)+'\n')
    print(json.dumps(dict(passed=True,approved=False,stored_polynomials=305,maximum_ulp_error=report['stored_execution']['maximum_ulp_error'])))
