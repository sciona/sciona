...
def verify(source):
    proposed=plan()
    ids=[proposed['artifact_id']]+[atom['artifact_id'] for atom in proposed['atoms']+proposed['reused_atoms']]
    with psycopg.connect(dotenv_values(ROOT/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row,
        ...
        require(counts['n']==29,'All artifacts must be served')
        require(db.execute('SELECT count(*) AS n FROM catalog_atoms_served WHERE atom_id=ANY(%s::uuid[])',
                (ids[1:],)).fetchone()['n']==28,'Each provider must be served once in legacy view')
    ...
    return dict(passed=True,served_atoms=28,newly_approved_atoms=12,reused_approved_atoms=16,served_cdgs=1,trust_tier=3,source_intake_remains_draft=True,
        version_id=proposed['version_id'],graph_sha256=proposed['graph_sha256'],scenarios=result['source_scenarios'],
...
if __name__=='__main__':
    ...
    result=verify(args.source_directory)
    (ROOT/'docs/reviews/nasa_domain_served_verification.json').write_text(json.dumps(result,indent=2)+'\n')
