"""Activate the qualified original Schwarzschild radius revision at automated Tier 3."""
import argparse
import json
...
from scripts.review_schwarzschild_revision import review,LIMITATIONS
...
RUNNER='schwarzschild-original-revision-community.v1'
DESCRIPTION='Compute the Schwarzschild length scale 2GM/c^2 in metres from positive finite SI masses and explicit positive scalar gravitational constant and light speed. Reuses one approved provider, preserves mass shape and immutable source proof history. Automated Tier 3 numerical realization with explicit physical interpretation limits.'
PLAIN_DESCRIPTION='Calculate a gravitational length scale for positive masses using supplied physical constants. Interpreting this length as an event horizon requires a spherical, nonrotating, uncharged system with the specified vacuum exterior. This computation does not classify objects as black holes or simulate collapse.'
REFERENCE='openstax-university-physics-v1-13-7'
DOMAIN='Positive finite SI mass and scalar constants; finite positive output. Horizon interpretation requires spherical nonrotating uncharged Schwarzschild regime with asymptotically flat vacuum exterior.'
...
def promote(source,apply=False):
    if apply:
        gates=json.loads((ROOT/'docs/reviews/schwarzschild_revision_publication_transaction.json').read_text())
        require(gates['passed'] and gates['post_activation_failure_rolled_back']
            and gates['publisher_sha256']==sha(__file__)
            and gates['validator_sha256']==sha(ROOT/'scripts/validate_schwarzschild_publication_transaction.py'),'Publication rollback qualification required')
    ...
    with psycopg.connect(dotenv_values(ROOT/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row,
        ...
        db.execute("SELECT pg_advisory_xact_lock(hashtext('schwarzschild-original-revision-draft.v1'))")
        ...
        bound=uuid5(version,RUNNER+':positive-mass-regime')
...
if __name__=='__main__':
    ...
    if args.apply:
        name='schwarzschild_revision_publication_repeat.json' if report['already_approved'] else 'schwarzschild_revision_publication.json'
