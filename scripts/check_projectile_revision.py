...
from scripts.stage_projectile_original_revision import ROOT,ARTIFACT,ORIGINAL,EXECUTION,history,sha,require
...
def check(db,approved=False):
    ...
    require(imported['stager_sha256']==sha(ROOT/'scripts/stage_projectile_original_revision.py'),'Stager changed')
    require(imported['scope_audit_sha256']==sha(ROOT/'docs/reviews/physics_projectile_original_scope.json'),'Scope evidence changed')
    ...
    require(len(bindings)==1 and bindings[0]['node_id']=='trajectory' and bindings[0]['status']=='active'
    ...
    approvals=db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='projectile-corrected-community.v1' AND passed",(EXECUTION,)).fetchall()
    require(len(approvals)==1,'Parent semantic approval missing')
    evidence=ROOT/'docs/reviews/projectile_execution.json'
    require(sha(evidence)==approvals[0]['details']['evidence_sha256']['projectile_execution.json'],'Parent evidence changed')
    qualified=json.loads(evidence.read_text())
    require(sha(inspect.getfile(projectile_trajectory))==qualified['provider_sha256'],'Provider source changed')
    require(list(inspect.signature(projectile_trajectory).parameters)==['x','x0','y0','vx0','vy0','gravity'],'Provider signature differs')
