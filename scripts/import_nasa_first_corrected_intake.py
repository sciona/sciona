...
from scripts.plan_nasa_first_corrected_intake import ROOT,plan,check_source_anchor,check_atoms,origin_rows,sha,require

RUNNER='nasa-first-corrected-intake-draft.v1'


def stage(apply=False):
    if apply:
        gate=json.loads((ROOT/'docs/reviews/competition_nasa_first_corrected_intake_draft_transaction.json').read_text())
        require(gate['passed'] and gate['failure_rollback_verified'] and gate['importer_sha256']==sha(Path(__file__))
            and gate['validator_sha256']==sha(ROOT/'scripts/validate_nasa_first_corrected_intake_draft_transaction.py'),'Draft transaction qualification missing or stale')
    proposed=plan();graph=build_nasa_first_complete_graph()
    ...
    with psycopg.connect(dotenv_values(ROOT/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row,
        ...
        for lock in ['available-feature-provider-draft.v1','nasa-first-lifecycle-provider-draft.v1',RUNNER]:
        ...
        for atom in proposed['atoms']:
            key=dict(dependent_version_id=version,dependency_artifact_fqdn=atom['fqdn'],dependency_content_hash=atom['content_hash'],port_name='')
            ensure('artifact_dependencies',key,dict(**key,dependency_role='logic_atom',optional=False,
                binding_metadata=Jsonb(dict(provider_version_id=atom['version_id'],runtime_fqdn=atom['runtime_fqdn'],
                    scope='Approved runtime dependency, including nested branch and controller-injected operations.'))))
        evidence=uuid5(version,RUNNER)
            ...
            details=Jsonb(dict(scope='Corrected execution version staged without approving or rewriting the original snapshot.',
                original_rows_sha256=proposed['original_rows_sha256'],evidence_sha256=proposed['evidence_sha256'],planner_sha256=proposed['planner_sha256'],nested_bindings=proposed['nested_bindings'],controller_dependency=proposed['controller_dependency'],output_bindings=proposed['output_bindings']))))
...
if __name__=='__main__':
    ...
    name='competition_nasa_first_corrected_intake_draft_rollback.json' if not args.apply else ('competition_nasa_first_corrected_intake_draft_import.json' if report['rows_created'] else 'competition_nasa_first_corrected_intake_draft_repeat.json')
