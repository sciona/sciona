...
import scripts.promote_nasa_first_lifecycle_providers as publisher
from scripts.review_nasa_first_lifecycle_publication import ROOT, plan, sha
from scripts.review_nasa_first_lifecycle_publication import require
...
def validate():
    ...
    require(not dry['already_approved'] and dry['rows_created'] == 176 and dry['mutations'] == 246,
            'Expected fresh publication writes differ')
    require(snapshot(proposed) == before, 'Dry rollback changed catalog')
    checked = []
    for step in [1, 88, 176, 177, 246]:
        try:
...
if __name__ == '__main__':
    report = validate()
    (ROOT/'docs/reviews/competition_nasa_first_lifecycle_publication_gates.json').write_text(json.dumps(report, indent=2)+'\n')
