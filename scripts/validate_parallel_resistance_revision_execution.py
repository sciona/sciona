"""Run actual stored parallel_resistance revision with immutable source and Decimal oracle."""
import argparse
import asyncio
import json
...
from scripts.stage_parallel_resistance_original_revision import ROOT,sha,require
...
from scripts.validate_parallel_resistance_immutable_source import validate as validate_source
import scripts.validate_parallel_resistance_execution as oracle
...
async def validate(source,approved=False):
    scope=audit(source)
    require(scope==json.loads((ROOT/'docs/reviews/physics_parallel_resistance_original_scope.json').read_text()),'Qualified scope changed')
    ...
    with patch.object(oracle,'build_parallel_resistance_execution',return_value=graph),patch.object(oracle,'validate_source',side_effect=validate_source):
    ...
    return dict(passed=True,approved=approved,catalog_mutations=0,version_id=imported['version_id'],graph_sha256=imported['graph_sha256'],
        original_history_preserved=True,stored_execution=execution,validator_sha256=sha(__file__),checker_sha256=sha(ROOT/'scripts/check_parallel_resistance_revision.py'),
        source_validator_sha256=sha(ROOT/'scripts/validate_parallel_resistance_immutable_source.py'),scope_audit_sha256=imported['scope_audit_sha256'],
        limitations=['Corrected ideal two-resistor parallel network with explicit common-node and current-orientation premises and independently justified zero-voltage extension; no literal source-rule parity or reactive impedance claim.',
...
if __name__=='__main__':
    ...
    name='parallel_resistance_revision_served_execution.json' if args.approved else 'parallel_resistance_revision_catalog_execution.json'
