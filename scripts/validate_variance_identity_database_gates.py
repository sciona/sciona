...
from scripts.stage_variance_identity_revisions import ROOT,sha,require,RUNNER
...
def cases(item):
    ...
    return [
        ('parent_binding_hash','UPDATE artifact_cdg_bindings SET bound_version_content_hash=%s WHERE version_id=%s',('0'*64,parent),'Variance parent binding differs'),
        ('parent_approval',"UPDATE artifact_audit_evidence SET passed=false WHERE version_id=%s AND runner_version='variance-corrected-community.v1'",(parent,),'Unique approved variance targets required'),
        ('parent_provenance','UPDATE artifact_dependencies SET optional=true WHERE dependent_version_id=%s',(parent,),'Variance parent provenance differs'),
...
def validate():
    items=json.loads((ROOT/'docs/reviews/variance_identity_revision_import.json').read_text())['graphs'];reports=[]
...
if __name__=='__main__':
    report=validate()
    (ROOT/'docs/reviews/variance_identity_database_gates.json').write_text(json.dumps(report,indent=2)+'\n')
