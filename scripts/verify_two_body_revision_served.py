...
from scripts.promote_two_body_revision import check_publication,DESCRIPTION,PLAIN_DESCRIPTION
from scripts.review_two_body_revision import review
...
async def verify(source):
    ...
    # Source proof and independent mpmath checks remain unchanged; no database state is mocked.
    ...
    require(execution['graph_digest']==imported['graph_sha256'] and execution['full_runner_cases']==5
        and execution['synthetic_numeric_cases']==208 and execution['maximum_ulp_error']<=1
        and execution['source_proof']['selected_source_equations_matched']==13
        and execution['source_proof']['selected_source_premises_matched']==6,'Fresh stored numerical execution differs')
    ...
    return dict(passed=True,approved=True,trust_tier=3,selection=before,execution=execution,catalog_mutations=0,
        validator_sha256=sha(__file__),execution_validator_sha256=sha(ROOT/'scripts/validate_two_body_execution.py'),
...
if __name__=='__main__':
    ...
    (ROOT/'docs/reviews/two_body_revision_served_verification.json').write_text(json.dumps(report,indent=2)+'\n')
    print(json.dumps(dict(passed=True,approved=True,trust_tier=3,stored_numeric_cases=208,maximum_ulp_error=report['execution']['maximum_ulp_error'])))
