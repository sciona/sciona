...
import scripts.promote_period_frequency_revision as publisher
...
def snapshot():
    with psycopg.connect(dotenv_values(ROOT/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row,
        ...
        check(db)
        result={}
        for table in ['artifacts','artifact_versions','artifact_descriptions','artifact_audit_rollups','artifact_audit_evidence','artifact_validity_bounds']:
...
if __name__=='__main__':
    ...
    (ROOT/'docs/reviews/period_frequency_revision_publication_transaction.json').write_text(json.dumps(report,indent=2)+'\n')
