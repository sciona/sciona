...
from scripts.plan_nasa_domain_draft import ROOT, audit, plan, check_reused

RUNNER = 'nasa-domain-draft.v1'
...
def stage(apply=False):
    ...
    with psycopg.connect(dotenv_values(ROOT / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        core_dependency = dict(dependent_version_id=version, dependency_artifact_fqdn=core['fqdn'],
                               dependency_content_hash=core['graph_sha256'], port_name='')
        ensure('artifact_dependencies', core_dependency, dict(**core_dependency, dependency_role='cdg', optional=False,
            binding_metadata=Jsonb(dict(scope='Mandatory approved numerical structure provenance; its shared atom bindings are invoked directly.',
                                       source_version_id=core['version_id']))))
        targets.append((identity, version))
    ...
    return dict(applied=apply, rows_created=created, draft_atoms=len(proposed['atoms']), draft_cdgs=1, approved=False,
                catalog_graph_roundtrip=True, reused_approved_atoms=len(proposed['reused_atoms']), graph_sha256=digest)
