...
from scripts.plan_nasa_population_drafts import BUILDERS
...
from scripts.plan_nasa_population_drafts import ROOT, audit, plan, check_parent
from scripts.review_conditional_correction import sha
from pathlib import Path

RUNNER = 'nasa-population-draft.v1'


def stage(apply=False):
    if apply:
        gates=json.loads((ROOT/'docs/reviews/nasa_population_draft_transaction_gates.json').read_text())
        require(gates['passed'] and gates['injected_failure_rolled_back']
            and gates['importer_sha256']==sha(Path(__file__))
            and gates['validator_sha256']==sha(ROOT/'scripts/validate_nasa_population_draft_transaction.py'),
            'Draft transaction qualification missing or stale')
    ...
    with psycopg.connect(dotenv_values(ROOT / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        for kind, selected_graph in proposed['graphs'].items():
            ...
            require(digest==selected_graph['graph_sha256'],'Population graph drift')
            ...
            require(_artifact_document_to_cdg(document,version_id=str(version),content_hash=digest,
                require_execution_envelope=True)==graph,'Catalog population serialization differs')
            targets.append((identity,version))
        for target,version in targets:
            evidence=uuid5(version,RUNNER)
                ...
                runner_version=RUNNER,details=Jsonb(dict(scope='Reviewed population draft import only; no publication approval',semantic_review=semantic))))
        ...
        check_parent(db,proposed['parent_lifecycle'])
        require(audit()==semantic,'Population evidence changed during transaction')
...
if __name__=='__main__':
    ...
    result=stage(args.apply)
    name='nasa_population_draft_rollback.json' if not args.apply else (
        'nasa_population_draft_import.json' if result['rows_created'] else 'nasa_population_draft_import_repeat.json')
