...
from scripts.stage_nasa_first_lifecycle_providers import RUNNER
...
def validate():
    ...
    return dict(passed=True,approved=False,committed_catalog_mutations=0,provider_count=35,
        rejected_faults=rejected,rollback_verified=True,
        validator_sha256=sha(Path(__file__)),plan_sha256=sha(ROOT/'docs/reviews/competition_nasa_first_lifecycle_provider_plan.json'),
...
if __name__=='__main__':
    result=validate()
    (ROOT/'docs/reviews/competition_nasa_first_lifecycle_database_gates.json').write_text(json.dumps(result,indent=2)+'\n')
