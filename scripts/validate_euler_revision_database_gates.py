"""Rollback-only corruption checks for the original Euler proof revision."""
import json
...
from scripts.stage_euler_original_revision import ROOT,ARTIFACT,ORIGINAL,EXECUTION,sha,require
...
def cases(version):
    return [
        ...
        ('root_units',"UPDATE artifact_io_specs SET dim_signature='T2' WHERE version_id=%s AND direction='output'",(version,),'Stored port contract differs'),
...
def validate():
    ...
    return dict(passed=True,approved=False,rollback_verified=True,committed_catalog_mutations=0,
        ...
        validator_sha256=sha(__file__),checker_sha256=sha(ROOT/'scripts/check_euler_revision.py'),
...
if __name__=='__main__':
    report=validate()
    (ROOT/'docs/reviews/euler_revision_database_gates.json').write_text(json.dumps(report,indent=2)+'\n')
