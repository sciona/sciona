...
from scripts.stage_available_feature_providers import RUNNER as STAGING_RUNNER
from scripts.validate_available_feature_database_gates import check_staged

RUNNER = 'available-feature-provider-community.v1'
REFERENCE = 'available-feature-pinned-source'


def promote(apply=False, fault_after=None):
    if apply:
        gate = json.loads((ROOT/'docs/reviews/available_feature_provider_publication_gates.json').read_text())
        require(gate['passed'] and gate['publisher_sha256'] == sha(Path(__file__))
                and gate['validator_sha256'] == sha(ROOT/'scripts/validate_available_feature_publication.py')
    ...
    with psycopg.connect(dotenv_values(ROOT/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        title = 'Pinned first-place source motivating reusable feature operations'
        ...
        for atom in proposed['atoms']:
            ...
            for table, key in [('artifact_references', 'artifact_id'), ('atom_references', 'atom_id')]:
                    ...
                    'confidence': 'high', 'relevance_note': 'Generic corrected operations with explicit caller policies, availability and units; retained source notice and synthetic transfer evidence.'})
        ...
        for view, key in [('catalog_artifacts_served', 'artifact_id'), ('catalog_atoms_served', 'atom_id')]:
            require(db.execute(f'SELECT count(DISTINCT {key}) AS n FROM {view} WHERE {key}=ANY(%s::uuid[])', (ids,)).fetchone()['n'] == 13,
    ...
    return dict(applied=apply, already_approved=already, rows_created=created, mutations=mutations,
                trust_tier=3, served_atoms_in_transaction=13, original_workflow_approved=False)
