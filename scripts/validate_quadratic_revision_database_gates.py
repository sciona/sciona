"""Rollback-only corruption checks for the original quadratic revision."""
import json
...
from scripts.stage_quadratic_original_revision import ROOT,ARTIFACT,ORIGINAL,EXECUTION,sha,require
...
def validate():
    ...
    return dict(passed=True,approved=False,rollback_verified=True,committed_catalog_mutations=0,
        ...
        validator_sha256=sha(__file__),checker_sha256=sha(ROOT/'scripts/check_quadratic_revision.py'),
...
if __name__=='__main__':
    report=validate()
    (ROOT/'docs/reviews/quadratic_revision_database_gates.json').write_text(json.dumps(report,indent=2)+'\n')
