...
from scripts.stage_quadratic_original_revision import ROOT,ARTIFACT,ORIGINAL,REPLAY,EXECUTION,history,sha,require
...
def check(db,approved=False):
    ...
    require(imported['stager_sha256']==sha(ROOT/'scripts/stage_quadratic_original_revision.py'),'Stager changed')
    require(imported['scope_audit_sha256']==sha(ROOT/'docs/reviews/physics_quadratic_original_scope.json'),'Scope evidence changed')
    ...
    require(len(bindings)==1 and bindings[0]['node_id']=='roots' and bindings[0]['status']=='active'
    ...
    evidence=ROOT/'docs/reviews/quadratic_corrected_execution.json'
    require(sha(evidence)==approvals[0]['details']['evidence_sha256']['quadratic_corrected_execution.json'],'Parent evidence changed')
    qualified=json.loads(evidence.read_text())
    require(sha(inspect.getfile(real_quadratic_roots))==qualified['provider_sha256'],'Provider source changed')
    require(list(inspect.signature(real_quadratic_roots).parameters)==['a','b','c'],'Provider signature differs')
