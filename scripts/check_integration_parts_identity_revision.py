...
import importlib
import json
from scripts.stage_integration_parts_identity_revisions import ROOT,history,revised_graph,sha,require
from sciona.services.execution_graph_codec import encode_execution_graph
from sciona.services.catalog_artifact_retrieval import _artifact_document_to_cdg


def check(db,family,approved=False):
    imported=json.loads((ROOT/'docs/reviews/integration_parts_identity_revision_import.json').read_text())
    require(imported['stager_sha256']==sha(ROOT/'scripts/stage_integration_parts_identity_revisions.py'),'Legacy stager changed')
    require(imported['scope_audit_sha256']==sha(ROOT/'docs/reviews/integration_parts_identity_execution_scope.json'),'Legacy scope evidence changed')
    items=[r for r in imported['graphs'] if r['family']==family]
    scopes=[r for r in json.loads((ROOT/'docs/reviews/integration_parts_identity_execution_scope.json').read_text())['graphs'] if r['family']==family]
    ...
    from scripts.audit_integration_parts_identity_scope import check_parent
    frozen=json.loads((ROOT/'docs/reviews/integration_parts_identity_execution_scope.json').read_text())
    require(frozen['auditor_sha256']==sha(ROOT/'scripts/audit_integration_parts_identity_scope.py') and frozen['parent_checker_sha256']==sha(ROOT/'scripts/check_integration_parts_identity_parent.py'),'Integration by parts scope/checker changed')
