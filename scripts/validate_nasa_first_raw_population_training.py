...
from scripts.synthetic_nasa_first_raw_population import population_table
...
def validate(source):
    ...
    end_train=str(time_start+pd.Timedelta(minutes=128*15));tables=[];raw_reports=[]
    for population in range(10):
        table,raw_report=population_table(population,time_start)
    ...
    return dict(passed=True,approved=False,catalog_mutations=0,synthetic_only=True,backend_version=catboost.__version__,numpy_version=np.__version__,pandas_version=pd.__version__,native_threads=1,
        ...
        implementation_sha256={p:hashlib.sha256((ROOT/p).read_bytes()).hexdigest() for p in ['scripts/validate_nasa_first_raw_population_training.py','scripts/synthetic_nasa_first_raw_population.py','sciona/nasa_first_feature_adapter.py','sciona/model_feature_policy.py','sciona/prediction_postprocessing.py','scripts/audit_nasa_first_training_closure.py','scripts/validate_nasa_first_native_training.py','scripts/validate_nasa_first_global_population.py','scripts/validate_nasa_first_prediction_arithmetic.py']},
        limitations=['Synthetic raw inputs with explicit repeated queries exercise all seven corrected feature families; this is workflow qualification, not competition-performance evidence. Fallback paths remain unqualified.',
...
if __name__=='__main__':
    ...
    (ROOT/'docs/reviews/competition_nasa_first_raw_population_training.json').write_text(json.dumps(report,indent=2)+'\n')
