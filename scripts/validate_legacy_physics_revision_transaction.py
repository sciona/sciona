...
from scripts.stage_legacy_physics_revisions import ROOT,stage,sha,require


def snapshot():
    scope=json.loads((ROOT/'docs/reviews/legacy_physics_execution_scope.json').read_text())
...
def validate(source):
    ...
    return dict(passed=True,rollback_verified=True,injected_failure_rollback_verified=True,
        ...
        draft=dry,stager_sha256=sha(ROOT/'scripts/stage_legacy_physics_revisions.py'),validator_sha256=sha(__file__))
...
if __name__=='__main__':
    ...
    (ROOT/'docs/reviews/legacy_physics_revision_transaction.json').write_text(json.dumps(report,indent=2)+'\n')
